...
// =============================
// ⚠️ BREAKING CHANGE: Hardcoded API keys removed for security
 ...
 * Priority: IndexedDB (CEX_API_KEYS) only - no localStorage fallback
 *
...
function getCEXCredentials(cexName) {
    try {
        // Read from IndexedDB only (single source of truth)
        ...
        // No keys found - user must configure in Settings
...
function migrateCEXKeysToIndexedDB() {
    try {
        ...
        // Check if IndexedDB already has keys (user configured via Settings)
        if (typeof getFromLocalStorage === 'function') {
        ...
        // Migrate from localStorage MULTI_* keys
        // ✅ CEX list matches CONFIG_CEX (6 active CEXs, BYBIT and LBANK disabled)
        const cexList = ['GATE', 'BINANCE', 'MEXC', 'KUCOIN', 'BITGET', 'INDODAX'];
        const migratedKeys = {};
        let migratedCount = 0;
        ...
        cexList.forEach(cex => {
            ...
            if (apiKey && secretKey) {
                ...
                // Add passphrase for KUCOIN and BITGET if exists
        ...
        // Save to IndexedDB if any keys found
        if (migratedCount > 0 && typeof saveToLocalStorage === 'function') {
            ...
            // Optional: Clean up old MULTI_* keys after successful migration
            // Uncomment if you want to remove old keys
            // cexList.forEach(cex => {
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log('[CEX Migration] ℹ️ No legacy API keys found to migrate'); } catch(_) {}
            localStorage.setItem('CEX_KEYS_MIGRATED', 'true');
        }
    } catch (error) {
...
// ✅ REMOVED: syncCEXKeysToLocalStorage() function
// localStorage sync is no longer needed - modal.html now reads directly from IndexedDB
...
if (typeof window !== 'undefined' && window.__IDB_LOCALSTORAGE_READY__) {
    window.__IDB_LOCALSTORAGE_READY__.then(() => {
        ...
        // ✅ localStorage sync removed - modal.html now reads directly from IndexedDB
    }).catch(err => {
        console.error('[CEX Migration] Failed to wait for IndexedDB:', err);
    });
}

// ⚠️ CEX_SECRETS now empty - user must configure keys in Settings
const CEX_SECRETS = {
...
try {
    if (typeof window !== 'undefined') {
        ...
        // ✅ syncCEXKeysToLocalStorage removed - no longer needed
    }
} catch (_) { }
...
function get0xApiKey() {
    try {
        ...
        if (typeof SavedSettingData !== 'undefined' && SavedSettingData && SavedSettingData.matchaApiKeys) {
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log('[0x API] Reading keys from SavedSettingData (cached)'); } catch(_) {}
        }
        // 2. Fallback: Read directly from IndexedDB/localStorage
            ...
            if (settings && settings.matchaApiKeys) {
                ...
                try { if (window.SCAN_LOG_ENABLED) console.log('[0x API] Reading keys from IndexedDB (direct read)'); } catch(_) {}
            }
        }
        ...
        if (keys.length > 0) {
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log(`[0x API] ✅ Using user API key (key ${zeroxKeyIndex}/${keys.length} total keys)`); } catch(_) {}
            return rotatedKey;
        }
