...
(function initIndexedDBStorage() {
    ...
    // ✅ OPTIMIZATION: IDB Write Batcher to reduce transaction overhead
    class IDBWriteBatcher {
        constructor(dbGetter, storeName) {
        ...
        async write(key, val) {
            return new Promise((resolve, reject) => {
                this.queue.push({ key, val, resolve, reject, type: 'put' });
        ...
        async delete(key) {
            return new Promise((resolve, reject) => {
                this.queue.push({ key, resolve, reject, type: 'delete' });
        ...
        async flush() {
            ...
            try {
                const currentDb = await this.dbGetter();
                // ✅ Single transaction for entire batch
                const tx = currentDb.transaction([this.storeName], 'readwrite');
                const store = tx.objectStore(this.storeName);

                // Execute all operations in single transaction
                batch.forEach(({ key, val, type }) => {
                    if (type === 'put') {
                        store.put({ key, val });
                    } else if (type === 'delete') {
                        store.delete(key);
            ...
            try {
                ...
                req.onsuccess = (ev) => {
                    ...
                    if (missing.length) {
                        ...
                        up.onsuccess = (e2) => {
                            db = e2.target.result;
                            // ✅ Initialize write batcher after DB is ready
                            if (!writeBatcher) {
                                writeBatcher = new IDBWriteBatcher(() => Promise.resolve(db), STORE_KV);
                            }
                            resolve(db);
                        };
    ...
    // ✅ OPTIMIZATION: Use write batcher for better performance
    async function idbSet(nsKey, val) {
        try {
            await openDB();
            // Use batcher if available, fallback to direct write
            if (writeBatcher) {
                return await writeBatcher.write(nsKey, val);
            }
    ...
    async function idbDel(nsKey) {
        try {
            await openDB();
            // Use batcher if available, fallback to direct delete
            if (writeBatcher) {
                return await writeBatcher.delete(nsKey);
            }
    ...
    // ✅ OPTIMIZATION: Warm cache selectively (only critical keys)
    function warmCacheSelective(keys) {
        return new Promise(async (resolve) => {
            try {
                await openDB();
    ...
    // Warm all cache entries early (best-effort) - LEGACY FALLBACK
    ...
    const criticalKeys = [
        ...
        (window.storagePrefix || '') + 'ALL_GAS_FEES',
        (window.storagePrefix || '') + 'PRICE_RATE_USDT',
        // ✅ TOKEN KEYS - Load all chain tokens on startup
        (window.storagePrefix || '') + 'TOKEN_MULTICHAIN',
        (window.storagePrefix || '') + 'TOKEN_BSC',
        (window.storagePrefix || '') + 'TOKEN_POLYGON',
        (window.storagePrefix || '') + 'TOKEN_ARBITRUM',
    ...
    try {
        // Warm critical keys first (fast)
        window.whenStorageReady = warmCacheSelective(criticalKeys).then(count => {
            try {
        ...
        // Fallback to full cache load
        window.whenStorageReady = warmCacheAll();
    }
    ...
    // ✅ OPTIMIZATION: Async version for proper IDB read (eliminates race condition)
    window.getFromLocalStorageAsync = async function (key, defaultValue) {
        try {
            ...
            // Await IDB fetch to ensure data is loaded before returning
            const val = await idbGet(nsKey);
            if (val !== undefined) {
                cache[nsKey] = val;
                return val;
            }
