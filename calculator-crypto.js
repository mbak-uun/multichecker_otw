...
(function() {
    ...
    function fetchRates() {
        $.when(
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log('✅ IDR rate updated:', ratePrice.idr); } catch(_) {}
        }).fail(function (error) {
            try { if (window.SCAN_LOG_ENABLED) console.log("-----------------------"); } catch(_) {}
            try { if (window.SCAN_LOG_ENABLED) console.log(error); } catch(_) {}
            ...
            if (storedRate > 0) {
                ...
                try { if (window.SCAN_LOG_ENABLED) console.log('ℹ️ Using stored IDR rate:', ratePrice.idr); } catch(_) {}
            }
        });
    }
    ...
    function enableCalculatorInputs() {
        ...
        try { if (window.SCAN_LOG_ENABLED) console.log('✅ Calculator inputs enabled'); } catch(_) {}
    }
    ...
    $(document).on('click', '#openCalculatorModal', function(event) {
        ...
        try {
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log('✅ Calculator modal opened'); } catch(_) {}
        } catch (error) {
    ...
    $('#openCalculatorModal').on('click', function(event) {
        ...
        try {
            ...
            enableCalculatorInputs();
            UIkit.modal('#calculator-modal').show();
            try { if (window.SCAN_LOG_ENABLED) console.log('✅ Calculator modal opened (direct handler)'); } catch(_) {}
        } catch (error) {
            console.error('❌ Error opening calculator modal:', error);
        }
    ...
    UIkit.util.on('#calculator-modal', 'shown', function () {
        enableCalculatorInputs();
        try { if (window.SCAN_LOG_ENABLED) console.log('✅ Calculator modal shown - inputs enabled'); } catch(_) {}
    });
    ...
    if (storedIDRRate > 0) {
        ...
        try { if (window.SCAN_LOG_ENABLED) console.log('✅ Loaded stored IDR rate on init:', ratePrice.idr); } catch(_) {}
    }
    ...
    $(document).on('keydown', function(event) {
        ...
        if ((event.ctrlKey || event.metaKey) && event.key === 'k') {
            ...
            event.stopPropagation();
            UIkit.modal('#calculator-modal').show();
            try { if (window.SCAN_LOG_ENABLED) console.log('✅ Calculator opened via keyboard shortcut (Ctrl/Cmd+K)'); } catch(_) {}
            return false;
        }
    });

    try { if (window.SCAN_LOG_ENABLED) console.log('✅ Calculator Crypto module loaded. Press Ctrl+K (or Cmd+K) to open calculator.'); } catch(_) {}
