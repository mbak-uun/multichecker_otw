...
(function() {
    ...
    async function loadWeb3Cache() {
        try {
            ...
            try { if (window.SCAN_LOG_ENABLED) console.warn('[Web3 Cache] Failed to load cache:', e); } catch(_) {}
        }
        return {};
    }
    ...
    async function saveWeb3Cache(cache) {
        try {
            ...
            try { if (window.SCAN_LOG_ENABLED) console.warn('[Web3 Cache] Failed to save cache:', e); } catch(_) {}
        }
    }
    ...
    async function fetchPriceMapForCex(cexName) {
        ...
        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
            try {
                try { if (window.SCAN_LOG_ENABLED) console.log(`[fetchPriceMapForCex] ${upper} - Attempt ${attempt}/${MAX_RETRIES}`); } catch(_) {}
                ...
                const attemptPromise = (async () => {
                    ...
                    if (map.size === 0) {
                        try { if (window.SCAN_LOG_ENABLED) console.warn(`[fetchPriceMapForCex] ${upper} - Parser returned empty map, data:`, data); } catch(_) {}
                ...
                try { if (window.SCAN_LOG_ENABLED) console.log(`[fetchPriceMapForCex] ${upper} - Success (${map.size} pairs)`); } catch(_) {}
    ...
    async function fetchWeb3TokenData(contractAddress, chainKey, web3Cache = null) {
        ...
        if (web3Cache) {
            ...
            if (cached) {
                try { if (window.SCAN_LOG_ENABLED) console.log(`[Web3 Cache] HIT for ${contract} on ${chainKey}`); } catch(_) {}
                return cached;
            }
        }
        // ==================================================
        ...
        if (WEB3_PENDING_REQUESTS.has(requestKey)) {
            try { if (window.SCAN_LOG_ENABLED) console.log(`[Web3 Dedup] Waiting for pending request: ${contract} on ${chainKey}`); } catch(_) {}
                ...
                try {
                    ...
                    try {
                        ...
                        if (isSolana) {
                            ...
                            try { if (window.SCAN_LOG_ENABLED) console.log(`[Solana] Got decimals for ${contract}: ${decimals}`); } catch(_) {}
                        }
    ...
    async function processSnapshotForCex(chainKey, selectedCex, perTokenCallback = null) {
        ...
        if (failedCexList.length > 0) {
            ...
            try { if (window.SCAN_LOG_ENABLED) console.warn(`[Snapshot] Partial success: ${successCount}/${orderedCex.length} CEX succeeded`, {
        ...
        try {
            ...
            try { if (window.SCAN_LOG_ENABLED) console.warn('[Snapshot] Failed to load settings:', e.message); } catch(_) {}
        }
        ...
        try { if (window.SCAN_LOG_ENABLED) console.log(`[Web3 Fetch] Config: BATCH_SIZE=${BATCH_SIZE}, BATCH_DELAY=${BATCH_DELAY}ms, WEB3_REQUEST_DELAY=${WEB3_REQUEST_DELAY}ms`); } catch(_) {}
        // ==========================================
        ...
        for (let batchStart = 0; batchStart < allTokens.length; batchStart += BATCH_SIZE) {
            ...
            if (batchEnd < allTokens.length && BATCH_DELAY > 0) {
                ...
                try { if (window.SCAN_LOG_ENABLED) console.log(`[Web3 Fetch] Batch ${batchNumber} selesai, jeda ${BATCH_DELAY}ms sebelum batch ${batchNumber + 1}`); } catch(_) {}
            }
        }
        ...
        for (let i = 0; i < activeCexList.length; i++) {
            ...
            try {
                ...
                try { if (window.SCAN_LOG_ENABLED) console.log(`✅ [${cexUpper}] Berhasil fetch ${priceMap.size} harga`); } catch(_) {}
        ...
        try { if (window.SCAN_LOG_ENABLED) console.log(`📊 [Fetch Harga] ${priceMapsByCex.size}/${activeCexList.length} CEX berhasil fetch harga`); } catch(_) {}
        ...
        if (priceEligibleTokens.length > 0) {
            ...
            for (const [cexName, tokenList] of tokensByCex.entries()) {
                ...
                if (!priceData || !priceData.map) {
                    ...
                    try { if (window.SCAN_LOG_ENABLED) console.warn(`[${cexName}] No price data available (failed in PHASE 1), skipping price assignment`); } catch(_) {}
                ...
                try { if (window.SCAN_LOG_ENABLED) console.log(`✅ [${cexName}] Assigned harga ke ${tokenList.length} token`); } catch(_) {}
            }
            ...
            if (typeof perTokenCallback === 'function' && enrichedTokens.length > 0) {
                try {
                    try { if (window.SCAN_LOG_ENABLED) console.log(`[Snapshot] Calling perTokenCallback with ${enrichedTokens.length} tokens (batch mode)`); } catch(_) {}
            ...
            if (enrichedTokens.length > 0) {
                ...
                try { if (window.SCAN_LOG_ENABLED) console.log(`📦 [Database] Loading existing tokens for ${chainKey}...`); } catch(_) {}
                ...
                try { if (window.SCAN_LOG_ENABLED) console.log(`📦 [Database] Existing tokens: ${existingTokensFull.length}, New tokens: ${enrichedTokens.length}`); } catch(_) {}
                ...
                try { if (window.SCAN_LOG_ENABLED) console.log(`📦 [Database] Saving ${mergedTokens.length} total tokens to IndexedDB...`); } catch(_) {}
                ...
                try { if (window.SCAN_LOG_ENABLED) console.log(`✅ [Database] ${summaryMsg}`); } catch(_) {}
            ...
            try {
                if (web3Cache) {
                    ...
                    try { if (window.SCAN_LOG_ENABLED) console.log('[Web3 Cache] Saved cache with', Object.keys(web3Cache).length, 'entries'); } catch(_) {}
                }
            } catch(e) {
                try { if (window.SCAN_LOG_ENABLED) console.warn('[Web3 Cache] Failed to save cache:', e); } catch(_) {}
            }
