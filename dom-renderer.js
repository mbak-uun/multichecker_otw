    ...
    dexList.forEach(dexKey => {
      ...
      if (found) {
        ...
        if (dexKeyLower === 'dzap' || dexKeyLower === 'lifi') {
          try {
            if (window.SCAN_LOG_ENABLED) console.log(`[buildDexSlots] ${dexKeyLower.toUpperCase()} cell rendering:`, {
              dexKeyLower,
...
function DisplayPNL(data) {
  ...
  try {
    if (el.dataset && el.dataset.final === '1' && el.classList.contains('dex-skip')) {
      try { if (window.SCAN_LOG_ENABLED) console.warn(`⚠️ [DisplayPNL] Cell ${elementId} sudah di-SKIP, tidak akan ditimpa dengan harga`); } catch (_) { }
  ...
  // ✅ FIX: Gunakan tooltipText dari data object (dikirim dari scanner.js handleSuccess)
  // Ini lebih reliable karena tidak bergantung pada timing DOM update
  let __titleLog = null;
  try {
    // Priority: 1. data.tooltipText (dari scanner.js), 2. existing titleLog (dari dataset)
    if (data.tooltipText) {
      __titleLog = String(data.tooltipText);
    } else if (el && el.dataset && el.dataset.titleLog) {
      __titleLog = String(el.dataset.titleLog);
    }
  } catch (_) { }
  // FIX: Set tooltip lengkap dari tooltipText
  try {
    if (__titleLog) {
      el.dataset.titleLog = __titleLog;
      el.setAttribute('title', __titleLog);
      // Also set on status span if exists
      const statusSpan = el.querySelector('.dex-status');
      if (statusSpan) statusSpan.setAttribute('title', __titleLog);
    }
  } catch (_) { }
  const $mainCell = $(el);
  ...
  if (data.autoLevelEnabled && data.autoVolResult && data.maxModal) {
    ...
    if (isInsufficientVolume) {
      try {
        if (window.SCAN_LOG_ENABLED) console.warn('⚠️  [DOM-RENDERER] Auto Level Insufficient:', {
          maxModal,
          actualModal,
    ...
    if (maxModal > 0) {
      try {
        ...
        try { if (window.SCAN_LOG_ENABLED) console.warn('[DOM-RENDERER] Failed to update modal display:', e); } catch (_) { }
      }
    ...
    if (data.cexBuyPriceDisplay) {
      try {
        if (window.SCAN_LOG_ENABLED) console.log('💵 [DOM-RENDERER] Override CEX BUY Price:', {
    ...
    if (data.cexSellPriceDisplay) {
      try {
        if (window.SCAN_LOG_ENABLED) console.log('💵 [DOM-RENDERER] Override CEX SELL Price:', {
  ...
  if (isMultiDex && Array.isArray(subResults) && subResults.length > 0) {
    try {
      ...
      try { if (window.SCAN_LOG_ENABLED) console.log(`[Multi-DEX Render] ${dexLabel}: Display max ${maxProviders} providers`); } catch (_) { }
      ...
      if (data.autoVolResult && data.maxModal) {
        ...
        if (!volumeSufficient) {
          try {
            if (window.SCAN_LOG_ENABLED) console.warn(`⚠️  [Multi-DEX] Signal SUPPRESSED due to insufficient orderbook volume:`, {
              dex: dextype,
      ...
      if (hasSignal && typeof InfoSinyal === 'function') {
        ...
        if (volumeSufficient) {
          try { if (window.SCAN_LOG_ENABLED) console.log(`✅ [Multi-DEX] Sending signal to: ${signalDexType}, Token: ${Name_in}->${Name_out}, PNL: ${bestPnl.toFixed(2)}`); } catch (_) { }
        } else {
          try {
            if (window.SCAN_LOG_ENABLED) console.warn(`⚠️  [Multi-DEX] Sending signal WITH WARNING (insufficient volume) for ${Name_in}->${Name_out} on ${dextype}:`, {
              bestPnl: bestPnl.toFixed(2),
        ...
        try {
          if (window.SCAN_LOG_ENABLED) console.log(`⚠️  [Multi-DEX] Signal NOT shown (insufficient volume) for ${Name_in}->${Name_out} on ${dextype}:`, {
            bestPnl: bestPnl.toFixed(2),
            maxModal: data.maxModal?.toFixed(2),
            actualModal: data.autoVolResult?.actualModal?.toFixed(2)
          });
        ...
        try {
          if (window.SCAN_LOG_ENABLED) console.log(`⚠️ [Multi-DEX] No signal (PNL ≤ 0) for ${Name_in}->${Name_out} on ${dextype}:`, {
            bestPnl: bestPnl.toFixed(2)
          });
        } catch (_) { }
        ...
        try {
          if (window.SCAN_LOG_ENABLED) console.log(`ℹ️ [Multi-DEX] Signal shown WITHOUT highlight for ${Name_in}->${Name_out} on ${dextype}:`, {
  ...
  if (data.autoVolEnabled) {
    ...
    if (!volOK) {
      ...
      try {
        if (window.SCAN_LOG_ENABLED) console.warn('⚠️  [AUTO VOL] Simple volume check failed:', {
          vol: vol.toFixed(2),
  ...
  if (data.autoLevelEnabled && data.autoVolResult && data.maxModal) {
    ...
    if (!volumeSufficient) {
      try {
        if (window.SCAN_LOG_ENABLED) console.warn('⚠️  [AUTO LEVEL] Orderbook volume insufficient:', {
          maxModal: maxModal.toFixed(2),
          actualModal: actualModal.toFixed(2),
          shortfall: (maxModal - actualModal).toFixed(2)
        });
  ...
  try {
    if (window.SCAN_LOG_ENABLED) console.log('🎯 [DisplayPNL] Price Analysis:', {
      dex: dextype,
      direction: trx,
  ...
  try {
    if (window.SCAN_LOG_ENABLED) console.log('💰 [DisplayPNL] Final Display Prices:', {
      dex: dextype,
      direction,
      Name_in,
      Name_out,
  ...
  if (hasSignal && typeof InfoSinyal === 'function') {
    // Log berbeda tergantung volume sufficiency
    if (volumeSufficient) {
      try { if (window.SCAN_LOG_ENABLED) console.log(`✅ [DisplayPNL] Sending signal for ${upper(dextype)}: ${upper(Name_in)}->${upper(Name_out)}, PNL: ${pnl.toFixed(2)}$`); } catch (_) { }
    } else {
      try {
        if (window.SCAN_LOG_ENABLED) console.warn(`⚠️  [DisplayPNL] Sending signal WITH WARNING (insufficient volume) for ${upper(Name_in)}->${upper(Name_out)} on ${upper(dextype)}:`, {
          pnl: pnl.toFixed(2),
          maxModal: data.maxModal?.toFixed(2),
          actualModal: data.autoVolResult?.actualModal?.toFixed(2)
        });
    ...
    try {
      if (window.SCAN_LOG_ENABLED) console.log(`⚠️ [DisplayPNL] No signal (PNL ≤ 0) for ${upper(Name_in)}->${upper(Name_out)} on ${upper(dextype)}:`, {
        pnl: pnl.toFixed(2)
      });
    } catch (_) { }
  } else if (!InfoSinyal) {
  ...
  if (hasSignal && !passSignal) {
    try {
      if (window.SCAN_LOG_ENABLED) console.log(`ℹ️ [DisplayPNL] Signal shown WITHOUT highlight for ${upper(Name_in)}->${upper(Name_out)} on ${upper(dextype)}:`, {
...
function InfoSinyal(DEXPLUS, TokenPair, PNL, totalFee, cex, NameToken, NamePair, profitLossPercent, modal, nameChain, codeChain, trx, idPrefix, domIdOverride, volumeSufficient = true) {
  ...
  if (!$container || $container.length === 0) {
    try { if (window.SCAN_LOG_ENABLED) console.warn(`❌ [InfoSinyal] Signal card container NOT FOUND for DEX: "${DEXPLUS}"`); } catch (_) { }
    try { if (window.SCAN_LOG_ENABLED) console.warn(`   ∟ Original: ${DEXPLUS}, Normalized: ${dexLowerKey}, Container: #sinyal${dexLowerKey}`); } catch (_) { }
    try { if (window.SCAN_LOG_ENABLED) console.warn(`   ∟ Token: ${NameToken}->${NamePair}, CEX: ${cex}, PNL: ${Number(PNL).toFixed(2)}, Chain: ${nameChain}`); } catch (_) { }
    try { if (window.SCAN_LOG_ENABLED) console.warn(`   ∟ Available:`, Array.from(document.querySelectorAll('[id^="sinyal"]')).map(el => el.id)); } catch (_) { }
  ...
  if (!existingSignal) {
    ...
    try { if (window.SCAN_LOG_ENABLED) console.log(`✅ [InfoSinyal] Signal added to DEX: ${DEXPLUS} (normalized: ${dexLowerKey}), Token: ${NameToken}->${NamePair}, PNL: ${Number(PNL).toFixed(2)}`); } catch (_) { }
  } else {
    ...
    try { if (window.SCAN_LOG_ENABLED) console.log(`🔄 [InfoSinyal] Signal updated for DEX: ${DEXPLUS}, Token: ${NameToken}->${NamePair}, PNL: ${Number(PNL).toFixed(2)}`); } catch (_) { }
  }
...
function calculateResult(baseId, tableBodyId, amount_out, FeeSwap, sc_input, sc_output, cex, Modal, amount_in, priceBuyToken_CEX, priceSellToken_CEX, priceBuyPair_CEX, priceSellPair_CEX, Name_in, Name_out, feeWD, dextype, nameChain, codeChain, trx, vol, DataDEX) {
  ...
  try {
    if (window.SCAN_LOG_ENABLED) console.log('📊 [calculateResult] PNL Calculation:', {
      dex: dextype,
      direction: trx,
      Name_in,
      Name_out,
