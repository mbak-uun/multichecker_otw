...
(function(global) {
    ...
    class OverlayManager {
        ...
        updateProgress(id, current, total, message = '') {
            ...
            if (!overlay) {
                try { if (window.SCAN_LOG_ENABLED) console.warn(`[OVERLAY MANAGER] updateProgress - overlay not found for id: ${id}`); } catch(_) {}
                try { if (window.SCAN_LOG_ENABLED) console.warn(`[OVERLAY MANAGER] Available overlay IDs:`, Array.from(this.overlays.keys())); } catch(_) {}
                return;
            }
    ...
    if (typeof global !== 'undefined') {
        ...
        document.addEventListener('keydown', function(e) {
            if (e.key === 'Escape' && instance.overlays.size > 0) {
                try { if (window.SCAN_LOG_ENABLED) console.log('[OVERLAY MANAGER] ESC pressed, hiding all overlays'); } catch(_) {}
                instance.hideAll();
            }
        });
    ...
    try { if (window.SCAN_LOG_ENABLED) console.log('[OVERLAY MANAGER] Module loaded'); } catch(_) {}
    try { if (window.SCAN_LOG_ENABLED) console.log('[OVERLAY MANAGER] Emergency cleanup: Type closeAllOverlays() in console or press ESC'); } catch(_) {}
