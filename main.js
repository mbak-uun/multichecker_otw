...
function refreshTokensTable() {
    ...
    // ✅ FIX: Set window.currentListOrderMulti BEFORE defer to prevent race conditions
    // Other handlers (search, filter changes) may access this variable before render callback
...
function renderSettingsForm() {
    ...
    try {
        ...
        if (s && typeof s === 'object' && s.JedaDexs) {
            ...
            // Get list of active DEX (not disabled, and either not Meta-DEX or META_DEX enabled)
            ...
            const activeDexKeys = Object.keys(CONFIG_DEXS || {}).filter(key => {
                ...
                return !dexConfig.disabled && (!dexConfig.isMetaDex || metaDexEnabled);
            });
            ...
            Object.keys(s.JedaDexs).forEach(dexKey => {
                ...
                if (!activeDexKeys.includes(dexKey) || forceRemoveDexs.includes(dexKey.toLowerCase())) {
                    try { if (window.SCAN_LOG_ENABLED) console.log(`[Settings Cleanup] Removing deleted/meta DEX: ${dexKey}`); } catch (_) { }
        ...
        try { if (window.SCAN_LOG_ENABLED) console.warn('[Settings Cleanup] Failed:', e.message); } catch (_) { }
    }

    // ✅ Generate DEX delay inputs - 100% dari CONFIG_DEXS (NO HARDCODE!)
    // Filter: disabled=false AND (isMetaDex=false OR META_DEX=true)
    // Include meta-DEX (lifi, rubic, rango, kamino) if META_DEX is enabled
    ...
    const activeDexList = Object.keys(CONFIG_DEXS || {})
        .filter(dexKey => {
            ...
            const isActive = !dexConfig.disabled && (!dexConfig.isMetaDex || metaDexEnabled);
            ...
            if (invalidKeys.includes(dexKey.toLowerCase())) {
                try { if (window.SCAN_LOG_ENABLED) console.warn(`[Settings] Skipping invalid DEX key: ${dexKey}`); } catch (_) { }
                return false;
            }
    ...
    try { if (window.SCAN_LOG_ENABLED) console.log('[Settings] Active DEX list from CONFIG_DEXS:', activeDexList); } catch (_) { }
    try { if (window.SCAN_LOG_ENABLED) console.log('[Settings] Total active DEX count:', activeDexList.length); } catch (_) { }
    ...
    activeDexList.forEach(dexKey => {
        ...
        const solanaBadge = dexConfig.solanaSupport ? ' <span style="font-size: 9px; background: #8417eaff; color: #f3efefff; padding: 2px 5px; border-radius: 3px; font-weight: bold;">SOLANA</span>' : '';
    ...
    try { if (window.SCAN_LOG_ENABLED) console.log('[SETTINGS LOAD] Full settings:', appSettings); } catch (_) { }
    try { if (window.SCAN_LOG_ENABLED) console.log('[SETTINGS LOAD] matchaApiKeys:', appSettings.matchaApiKeys); } catch (_) { }
    ...
    if (appSettings.matchaApiKeys) {
        ...
        try { if (window.SCAN_LOG_ENABLED) console.log('[SETTINGS LOAD] Keys to display:', keysForDisplay); } catch (_) { }
        $('#matchaApiKeys').val(keysForDisplay);
        try { if (window.SCAN_LOG_ENABLED) console.log('[SETTINGS LOAD] Field value after set:', $('#matchaApiKeys').val()); } catch (_) { }
    } else {
        try { if (window.SCAN_LOG_ENABLED) console.log('[SETTINGS LOAD] No matchaApiKeys found, setting empty'); } catch (_) { }
        $('#matchaApiKeys').val('');
    }

    // ✅ Verify field exists in DOM
    try { if (window.SCAN_LOG_ENABLED) console.log('[SETTINGS LOAD] matchaApiKeys field exists:', $('#matchaApiKeys').length > 0); } catch (_) { }
    ...
    // ✅ Generate and load CEX API Keys dynamically from CONFIG_CEX
    renderCEXAPIKeyInputs();
}

/**
 * ✅ NEW: Dynamically render CEX API key input fields based on CONFIG_CEX
...
function renderCEXAPIKeyInputs() {
    ...
    if (!container) {
        try { if (window.SCAN_LOG_ENABLED) console.warn('[CEX Settings] Container #cex-api-keys-container not found'); } catch (_) { }
        return;
    }

    // Get all CEX from CONFIG_CEX
        ...
        html += `
            <div>
              <div class="uk-card uk-card-small uk-card-body"
                style="background: ${hexToRgba(color)}; border-left: 3px solid ${color};">
                <div class="uk-text-small uk-text-bold uk-margin-small-bottom" style="color: ${color};">
                  <span uk-icon="icon: credit-card; ratio: 0.8"></span> ${cex}
                </div>
                <input type="text" class="uk-input uk-form-small uk-margin-small-bottom" id="cex_apikey_${cex}"
                  placeholder="API Key" aria-label="${cex} API Key">
                <input type="password" class="uk-input uk-form-small${needsPassphrase ? ' uk-margin-small-bottom' : ''}" id="cex_secret_${cex}" placeholder="Secret Key"
                  aria-label="${cex} Secret">
                ${needsPassphrase ? `<input type="password" class="uk-input uk-form-small" id="cex_passphrase_${cex}"
    ...
    try { if (window.SCAN_LOG_ENABLED) console.log(`[CEX Settings] ✅ Generated ${cexList.length} CEX input field(s):`, cexList); } catch (_) { }

    // Load saved values after rendering
    loadCEXApiKeys();
}
...
function loadCEXApiKeys() {
    try {
        ...
        if (loadedCount > 0) {
            try { if (window.SCAN_LOG_ENABLED) console.log(`[CEX Settings] ✅ Loading ${loadedCount} CEX API key(s):`, Object.keys(cexKeys)); } catch (_) { }
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log('[CEX Settings] ℹ️ No CEX API keys configured yet'); } catch (_) { }
        }
    } catch (error) {
...
function bootApp() {
    ...
    try {
        ...
        if (s && typeof s === 'object' && s.JedaDexs) {
            ...
            if (s.JedaDexs['fly']) {
                try { if (window.SCAN_LOG_ENABLED) console.warn('[Cleanup] ⚠️ Found legacy "fly" key in JedaDexs - REMOVING...'); } catch (_) { }
                delete s.JedaDexs['fly'];
                saveToLocalStorage('SETTING_SCANNER', s);
                try { if (window.SCAN_LOG_ENABLED) console.log('[Cleanup] ✅ "fly" removed successfully'); } catch (_) { }
            }
            try { if (window.SCAN_LOG_ENABLED) console.log('[Cleanup] JedaDexs keys before:', beforeKeys); } catch (_) { }
            try { if (window.SCAN_LOG_ENABLED) console.log('[Cleanup] JedaDexs keys after:', Object.keys(s.JedaDexs)); } catch (_) { }
        }
    } catch (e) {
    ...
    try {
        ...
        if (s && typeof s === 'object' && s.JedaDexs) {
            ...
            // Get list of active DEX from CONFIG_DEXS (disabled=false AND (isMetaDex=false OR META_DEX=true))
            const metaDexEnabled = (CONFIG_APP && CONFIG_APP.APP && CONFIG_APP.APP.META_DEX === true);
            const activeDexKeys = Object.keys(CONFIG_DEXS || {}).filter(key => {
                const cfg = CONFIG_DEXS[key];
                return !cfg.disabled && (!cfg.isMetaDex || metaDexEnabled);
            });

            try { if (window.SCAN_LOG_ENABLED) console.log('[Cleanup] Active DEX from CONFIG_DEXS:', activeDexKeys); } catch (_) { }
            try { if (window.SCAN_LOG_ENABLED) console.log('[Cleanup] Current JedaDexs keys:', Object.keys(s.JedaDexs)); } catch (_) { }
            ...
            Object.keys(s.JedaDexs).forEach(dexKey => {
                ...
                if (isInvalid || isInactive) {
                    try { if (window.SCAN_LOG_ENABLED) console.warn(`[Cleanup] ⚠️ Removing ${isInvalid ? 'INVALID' : 'INACTIVE'} DEX: ${dexKey}`); } catch (_) { }
                    delete s.JedaDexs[dexKey];
            ...
            if (hasChanges) {
                saveToLocalStorage('SETTING_SCANNER', s);
                try { if (window.SCAN_LOG_ENABLED) console.log(`[Cleanup] ✅ Removed ${removedKeys.length} invalid DEX:`, removedKeys); } catch (_) { }
                try { if (window.SCAN_LOG_ENABLED) console.log('[Cleanup] ✅ Final JedaDexs keys:', Object.keys(s.JedaDexs)); } catch (_) { }
            } else {
                try { if (window.SCAN_LOG_ENABLED) console.log('[Cleanup] ✅ No cleanup needed - all DEX keys are valid'); } catch (_) { }
            }
        }
    } catch (e) {
        try { if (window.SCAN_LOG_ENABLED) console.warn('[Cleanup] Failed to clean up inactive DEX:', e.message); } catch (_) { }
    }
...
async function deferredInit() {
    ...
    function renderFilterCard() {
        ...
        if (m.mode === 'multi') {
            ...
            const metaDexEnabled = (CONFIG_APP && CONFIG_APP.APP && CONFIG_APP.APP.META_DEX === true);
            const renderedDexKeys = new Set(); // ✅ FIX: Track rendered DEX to prevent duplicates
            Object.keys(CONFIG_DEXS || {}).forEach(dx => {
                ...
                // ✅ FIX: Skip if already rendered (prevent duplicates)
                if (renderedDexKeys.has(key)) {
                    try { if (window.SCAN_LOG_ENABLED) console.warn(`[Horizontal Filter] Skipping duplicate DEX: ${key}`); } catch (_) { }
                    return;
                }
                renderedDexKeys.add(key);

                const id = `fc-dex-${key}`;
                const cnt = byDex[key] || 0;

                // ✅ FIX: Tampilkan Meta-DEX meskipun count=0 (agar user bisa enable untuk scan berikutnya)
                // Regular DEX: skip jika count=0 (tidak ada data)
                if (!dexConfig.isMetaDex && cnt === 0) return;
            ...
            // ✅ Build DEX list: Base DEXs from CONFIG_CHAINS + Meta-aggregators (if enabled)
            let dexAllowed = ((CONFIG_CHAINS[chain] || {}).DEXS || []).map(x => String(x).toLowerCase());

            // ✅ Add meta-aggregators if META_DEX enabled
            const metaDexEnabled5 = (CONFIG_APP && CONFIG_APP.APP && CONFIG_APP.APP.META_DEX === true);
            if (metaDexEnabled5 && CONFIG_DEXS) {
                const isSolana = String(chain).toLowerCase() === 'solana';
                Object.keys(CONFIG_DEXS).forEach(dexKey => {
                    const dexConfig = CONFIG_DEXS[dexKey];
                    if (!dexConfig.isMetaDex || dexConfig.disabled) return;

                    // ✅ Filter by chain compatibility
                    if (isSolana && dexConfig.solanaSupport) {
                        dexAllowed.push(String(dexKey).toLowerCase());
                    } else if (!isSolana && dexConfig.evmOnly) {
                        dexAllowed.push(String(dexKey).toLowerCase());
                    }
                });
            }
    ...
    function renderFilterCardToModal() {
        ...
        if (m.mode === 'multi') {
            ...
            // ✅ Filter: Hanya tampilkan DEX asli (not disabled, not Meta-DEX)
            // Meta-DEX hanya muncul jika CONFIG_APP.APP.META_DEX === true
            const metaDexEnabled2 = (CONFIG_APP && CONFIG_APP.APP && CONFIG_APP.APP.META_DEX === true);
            const renderedDexKeys = new Set(); // ✅ FIX: Track rendered DEX to prevent duplicates
            Object.keys(CONFIG_DEXS || {}).forEach(dx => {
                const dexConfig = CONFIG_DEXS[dx];
                // Skip jika disabled
                if (dexConfig.disabled) return;
                // Skip Meta-DEX jika META_DEX disabled
                if (dexConfig.isMetaDex && !metaDexEnabled2) return;

                const key = String(dx).toLowerCase();

                // ✅ FIX: Skip if already rendered (prevent duplicates)
                if (renderedDexKeys.has(key)) {
                    try { if (window.SCAN_LOG_ENABLED) console.warn(`[Modal Filter] Skipping duplicate DEX: ${key}`); } catch (_) { }
                    return;
                }
                renderedDexKeys.add(key);
            ...
            // ✅ Add meta-aggregators if META_DEX enabled
            const metaDexEnabled3 = (CONFIG_APP && CONFIG_APP.APP && CONFIG_APP.APP.META_DEX === true);
            if (metaDexEnabled3 && CONFIG_DEXS) {
                const isSolana = String(chain).toLowerCase() === 'solana';
                Object.keys(CONFIG_DEXS).forEach(dexKey => {
                    const dexConfig = CONFIG_DEXS[dexKey];
                    if (!dexConfig.isMetaDex || dexConfig.disabled) return;
    ...
    // ✅ FIX: Unbind previous handler to prevent double-binding
    $('#ScannerFilterModal').off('click').on('click', function (e) {
        e.preventDefault();
    ...
    window.debugMultichainTokens = function (chainFilter = null) {
        try {
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log(`[DEBUG] Total tokens in multichain: ${multi.length}`); } catch (_) { }

            if (chainFilter) {
                const filtered = multi.filter(t => String(t.chain).toLowerCase() === String(chainFilter).toLowerCase());
                try { if (window.SCAN_LOG_ENABLED) console.log(`[DEBUG] Tokens for chain "${chainFilter}": ${filtered.length}`); } catch (_) { }
    ...
    try { if (window.SCAN_LOG_ENABLED) console.log('💡 Debug helper loaded: window.debugMultichainTokens() or window.debugMultichainTokens("ethereum")'); } catch (_) { }
    // ========== END DEBUG HELPER ==========
    ...
    function updatePriceFilterState() {
        try {
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log(`[updatePriceFilterState] Rows: ${rowCount}, HasData: ${hasData}, Filters: ${hasData ? 'ENABLED' : 'DISABLED'}`); } catch (_) { }
        } catch (e) {
    ...
    function updateSyncSelectedCount() {
        try {
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log(`[updateSyncSelectedCount] Counter updated: ${total} koin dipilih`); } catch (_) { }
            ...
            if ($pairRadios.length) {
                ...
                try { if (window.SCAN_LOG_ENABLED) console.log(`[updateSyncSelectedCount] Pair radios: ${hasSelection ? 'ENABLED' : 'DISABLED'}`); } catch (_) { }
            }
    ...
    async function fetchTickerMapForCex(cex) {
        ...
        if (SYNC_TICKER_PENDING.has(key)) {
            try { if (window.SCAN_LOG_ENABLED) console.log(`[fetchTickerMapForCex] Waiting for pending ${key} request...`); } catch (_) { }
        ...
        const fetchPromise = (async () => {
            try {
                try { if (window.SCAN_LOG_ENABLED) console.log(`[fetchTickerMapForCex] Fetching ${key} ticker data...`); } catch (_) { }
                ...
                try { if (window.SCAN_LOG_ENABLED) console.log(`[fetchTickerMapForCex] ${key} ticker data cached (${map.size} pairs)`); } catch (_) { }
                return map;
            } finally {
    ...
    async function loadSnapshotRecords(chainKey) {
        try {
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log('[SYNC DEBUG] loadSnapshotRecords - snapshotMap keys:', snapshotMap ? Object.keys(snapshotMap) : 'null'); } catch (_) { }

            if (!snapshotMap || typeof snapshotMap !== 'object') {
                try { if (window.SCAN_LOG_ENABLED) console.warn('[SYNC DEBUG] loadSnapshotRecords - No snapshot map found'); } catch (_) { }
                return [];
            }
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log('[SYNC DEBUG] loadSnapshotRecords - Looking for keys:', { keyLower, fallbackKey, availableKeys: Object.keys(snapshotMap) }); } catch (_) { }
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log('[SYNC DEBUG] loadSnapshotRecords - Found array length:', arr.length); } catch (_) { }

            if (!Array.isArray(arr) || !arr.length) {
                try { if (window.SCAN_LOG_ENABLED) console.warn('[SYNC DEBUG] loadSnapshotRecords - Empty array for chain:', chainKey); } catch (_) { }
                return [];
            }
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log('[SYNC DEBUG] Normalization stats:', { total: arr.length, valid: out.length, skippedNorm, skippedDedup }); } catch (_) { }
    ...
    async function loadSyncTokensFromSnapshot(chainKey, silent = false) {
        const key = String(chainKey || '').toLowerCase();
        try { if (window.SCAN_LOG_ENABLED) console.log('[SYNC DEBUG] loadSyncTokensFromSnapshot called for:', key); } catch (_) { }
        const raw = await loadSnapshotRecords(key);
        try { if (window.SCAN_LOG_ENABLED) console.log('[SYNC DEBUG] loadSyncTokensFromSnapshot - raw.length:', raw.length); } catch (_) { }
        if (!raw.length) {
            try { if (window.SCAN_LOG_ENABLED) console.warn('[SYNC DEBUG] loadSyncTokensFromSnapshot - Empty raw, returning false'); } catch (_) { }
            if (!silent) throw new Error('Snapshot kosong untuk chain ini.');
            return false; // Return false instead of throwing when silent
        }
        const savedTokens = getTokensChain(chainKey);
        try { if (window.SCAN_LOG_ENABLED) console.log('[SYNC DEBUG] loadSyncTokensFromSnapshot - calling setSyncModalData'); } catch (_) { }
        setSyncModalData(chainKey, raw, savedTokens, 'Snapshot');
        try { if (window.SCAN_LOG_ENABLED) console.log('[SYNC DEBUG] loadSyncTokensFromSnapshot - returning true'); } catch (_) { }
        if (!silent) {
    ...
    $(document).on('click', '#sync-tokens-btn', async function () {
        ...
        try {
            ...
            // Save to IndexedDB
            try { if (window.SCAN_LOG_ENABLED) console.log('[SYNC DEBUG] Saving to snapshot...', { chain: activeSingleChainKey, tokenCount: rawTokens.length }); } catch (_) { }
            const saveResult = await window.SnapshotModule.saveToSnapshot(activeSingleChainKey, rawTokens);
            try { if (window.SCAN_LOG_ENABLED) console.log('[SYNC DEBUG] Save result:', saveResult); } catch (_) { }

            // Load to modal
            const loaded = await loadSyncTokensFromSnapshot(activeSingleChainKey, true);
            try { if (window.SCAN_LOG_ENABLED) console.log('[SYNC DEBUG] Load result:', loaded); } catch (_) { }
    ...
    $(document).on('change', 'input[name="sync-price-filter"]', async function () {
        ...
        try { if (window.SCAN_LOG_ENABLED) console.log('[Price Filter] Changed to:', filterValue); } catch (_) { }
        ...
        if (filterValue === 'with-price') {
            ...
            try {
                ...
                try { if (window.SCAN_LOG_ENABLED) console.log(`[Price Filter] Fetching ALL prices from ${selectedCexs.length} CEX(s):`, selectedCexs); } catch (_) { }
                ...
                for (const cex of selectedCexs) {
                    try {
                        ...
                        if (window.App && window.App.Services && window.App.Services.CEX && typeof window.App.Services.CEX.fetchAllCEXPrices === 'function') {
                            ...
                            try { if (window.SCAN_LOG_ENABLED) console.log(`[Price Filter] ✅ ${cex}: Fetched ${fetchedCount} prices via ticker API`); } catch (_) { }
                            ...
                            try { if (window.SCAN_LOG_ENABLED) console.log(`[Price Filter] 📝 ${cex}: Updated ${updatedForThisCex} tokens with prices`); } catch (_) { }
                        }
                    } catch (error) {
                ...
                try { if (window.SCAN_LOG_ENABLED) console.log(`[Price Filter] ✅ SUMMARY: Fetched ${totalFetched} total prices, updated ${totalUpdated} tokens`); } catch (_) { }
    ...
    $(document).on('change', '#sync-filter-pair input[type="radio"]', function () {
        ...
        try { if (window.SCAN_LOG_ENABLED) console.log('[Pair Change] Selected pair:', selectedPair, '(HANYA untuk save, BUKAN fetch harga)'); } catch (_) { }
        ...
        try { if (window.SCAN_LOG_ENABLED) console.log('[Pair Change] No table re-render needed - harga tetap pakai USDT'); } catch (_) { }
    });
    ...
    $(document).on('change', 'input[name="sync-pick-mode"]', function () {
        ...
        try { if (window.SCAN_LOG_ENABLED) console.log(`[Sync Pick Mode] Mode: ${mode}, Found ${$allBoxes.length} checkboxes.`); } catch (_) { }
        ...
        if (mode === 'all') {
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log('[Sync Pick Mode] Mencentang semua checkbox...'); } catch (_) { }
            $allBoxes.prop('checked', true);

        } else if (mode === 'selected') {
            // 2. DIPILIH: Centang koin yang punya badge [DIPILIH] di kolom CEX
            try { if (window.SCAN_LOG_ENABLED) console.log('[Sync Pick Mode] Mencari koin dengan badge [DIPILIH]...'); } catch (_) { }
            let checkedCount = 0;
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log('[Sync Pick Mode] Koin [DIPILIH] ditemukan:', checkedCount); } catch (_) { }

        } else if (mode === 'snapshot') {
            // 3. SNAPSHOT: Centang koin yang punya badge [SNAPSHOT] di kolom CEX
            try { if (window.SCAN_LOG_ENABLED) console.log('[Sync Pick Mode] Mencari koin dengan badge [SNAPSHOT]...'); } catch (_) { }
            let snapshotCount = 0;
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log('[Sync Pick Mode] Koin [SNAPSHOT] ditemukan:', snapshotCount); } catch (_) { }

        } else if (mode === 'clear') {
            // 4. HAPUS: Uncheck semua koin
            try { if (window.SCAN_LOG_ENABLED) console.log('[Sync Pick Mode] Menghapus semua centangan...'); } catch (_) { }
        ...
        try { if (window.SCAN_LOG_ENABLED) console.log(`[Sync Pick Mode] Total checkbox tercentang setelah update: ${finalCount}`); } catch (_) { }
        // ===================================================================================
    });
...
$(document).ready(function () {
    ...
    window.renderSyncTable = function (chainKey) {
        ...
        try { if (window.SCAN_LOG_ENABLED) console.log('[renderSyncTable] Loading overlay created:', overlayId); } catch (_) { }
        // ===========================================
        ...
        setTimeout(() => {
            try {
                ...
                if (overlayId && window.AppOverlay) {
                    try { if (window.SCAN_LOG_ENABLED) console.log('[renderSyncTable] Hiding overlay due to error:', overlayId); } catch (_) { }
                    window.AppOverlay.hide(overlayId);
                }
            }
        }, 50);
    ...
    function renderSyncTableCore(chainKey, overlayId) {
        try {
            ...
            if (totalFiltered > MAX_SYNC_ROWS) {
                try { if (window.SCAN_LOG_ENABLED) console.warn(`[renderSyncTableCore] Too many rows (${totalFiltered}). Limiting to ${MAX_SYNC_ROWS} rows to prevent browser freeze.`); } catch (_) { }
            ...
            if ($modeRadios.length) {
                ...
                try { if (window.SCAN_LOG_ENABLED) console.log('[renderSyncTable] Radio buttons:', hasTableData ? 'ENABLED' : 'DISABLED', '- Table rows:', $('#sync-modal-tbody tr').length); } catch (_) { }
            }
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log('[renderSyncTableCore] Finally block executed. overlayId:', overlayId, 'AppOverlay exists:', !!window.AppOverlay); } catch (_) { }

            // Immediate hide instead of setTimeout to prevent event loop blocking
            if (overlayId && window.AppOverlay) {
                try { if (window.SCAN_LOG_ENABLED) console.log('[renderSyncTableCore] Calling AppOverlay.hide for:', overlayId); } catch (_) { }
                try {
                    window.AppOverlay.hide(overlayId);
                    try { if (window.SCAN_LOG_ENABLED) console.log('[renderSyncTableCore] AppOverlay.hide called successfully'); } catch (_) { }
                } catch (err) {
                ...
                setTimeout(() => {
                    ...
                    if (overlayElement) {
                        try { if (window.SCAN_LOG_ENABLED) console.warn('[renderSyncTableCore] FORCE REMOVING stuck overlay element:', overlayId); } catch (_) { }
                ...
                try { if (window.SCAN_LOG_ENABLED) console.warn('[renderSyncTableCore] Cannot hide overlay - overlayId:', overlayId, 'AppOverlay:', !!window.AppOverlay); } catch (_) { }
            }
            // ==========================================
        }
    }
...
(function initBulkModalEditor() {
    ...
    function updateBulkModalIconVisibility() {
        try {
            ...
            if (!$icon.length) {
                try { if (window.SCAN_LOG_ENABLED) console.warn('[Bulk Modal] Icon #BulkModalScanner not found'); } catch (_) { }
                return;
            }

            if (mode.type === 'single') {
                $icon.show(); // Show in single chain mode
                try { if (window.SCAN_LOG_ENABLED) console.log('[Bulk Modal] Icon visible (Single Chain mode)'); } catch (_) { }
            } else {
                $icon.hide(); // Hide in multichain mode
                try { if (window.SCAN_LOG_ENABLED) console.log('[Bulk Modal] Icon hidden (Multichain mode)'); } catch (_) { }
            }
        } catch (e) {
    ...
    UIkit.util.on('#bulk-modal-editor', 'hidden', async function () {
        try { if (window.SCAN_LOG_ENABLED) console.log('[Bulk Modal] 🔄 Modal closing - flushing all pending writes to IndexedDB...'); } catch (_) { }
        try {
            if (window.__IDB_FLUSH_PENDING__) {
                await window.__IDB_FLUSH_PENDING__();
                try { if (window.SCAN_LOG_ENABLED) console.log('[Bulk Modal] ✅ All pending writes flushed successfully on modal close'); } catch (_) { }
            }
        } catch (e) {
    ...
    async function initBulkEditor() {
        ...
        let dexList = chainConfig.DEXS || [];

        // ✅ Add meta-aggregators if META_DEX enabled
        const metaDexEnabled4 = (CONFIG_APP && CONFIG_APP.APP && CONFIG_APP.APP.META_DEX === true);
        if (metaDexEnabled4 && CONFIG_DEXS) {
            const isSolana = String(chainKey).toLowerCase() === 'solana';
            Object.keys(CONFIG_DEXS).forEach(dexKey => {
                const dexConfig = CONFIG_DEXS[dexKey];
                if (!dexConfig.isMetaDex || dexConfig.disabled) return;
                if (dexList.includes(dexKey)) return; // Skip if already in list
    ...
    async function saveLastProfileIndex(chainKey, index) {
        try {
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log(`[Bulk Modal] Saved last profile index ${index} for chain: ${chainKey}`); } catch (_) { }
        } catch (e) {
    ...
    async function loadProfiles(chainKey) {
        try {
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log(`[Bulk Modal] ✅ Loaded ${profiles.length} profiles for chain: ${chainKey}`); } catch (_) { }
            if (profiles.length > 0) {
                try { if (window.SCAN_LOG_ENABLED) console.log('[Bulk Modal] Profile names:', profiles.map(p => p.name).join(', ')); } catch (_) { }
            }
            return profiles;
        } catch (e) {
    ...
    async function saveProfiles(chainKey, profiles) {
        try {
            try { if (window.SCAN_LOG_ENABLED) console.log(`[Bulk Modal] 🔄 Saving ${profiles.length} profiles for chain: ${chainKey}`); } catch (_) { }
            ...
            if (verification.length === profiles.length) {
                try { if (window.SCAN_LOG_ENABLED) console.log(`[Bulk Modal] ✅ Save VERIFIED for chain: ${chainKey}`); } catch (_) { }
                return true;
            } else {
    ...
    async function populateProfileSelect() {
        const chainKey = bulkState.chain;
        if (!chainKey) {
            try { if (window.SCAN_LOG_ENABLED) console.warn('[Bulk Modal] ⚠️ Cannot populate profiles - chain key is missing'); } catch (_) { }
            return;
        }

        try { if (window.SCAN_LOG_ENABLED) console.log(`[Bulk Modal] 🔄 Populating profile dropdown for chain: ${chainKey}`); } catch (_) { }

        const profiles = await loadProfiles(chainKey);
        const $select = $('#profile-select');
        $select.find('option:not(:first)').remove();

        try { if (window.SCAN_LOG_ENABLED) console.log(`[Bulk Modal] 📝 Adding ${profiles.length} profiles to dropdown`); } catch (_) { }
        ...
        try { if (window.SCAN_LOG_ENABLED) console.log(`[Bulk Modal] 🔍 Last profile index for ${chainKey}: ${lastIndex}`); } catch (_) { }
        ...
        if (lastIndex !== null && lastIndex >= 0 && lastIndex < profiles.length) {
            ...
            if (profile) {
                ...
                try { if (window.SCAN_LOG_ENABLED) console.log(`[Bulk Modal] ✅ Auto-loaded profile "${profile.name}" for chain: ${chainKey}`); } catch (_) { }
                if (typeof toast !== 'undefined' && toast.info) {
                    toast.info(`Profil "${profile.name}" dimuat otomatis`);
                }
            }
        } else {
            try { if (window.SCAN_LOG_ENABLED) console.log(`[Bulk Modal] ℹ️ No previous profile to auto-load for ${chainKey}`); } catch (_) { }
        }
    }
    ...
    $(document).on('click', '#profile-save-btn', async function () {
        ...
        try { if (window.SCAN_LOG_ENABLED) console.log('[Bulk Modal] 🔍 Verifying profiles saved to IndexedDB...'); } catch (_) { }
        ...
        try { if (window.SCAN_LOG_ENABLED) console.log(`[Bulk Modal] 📦 Verification: ${verifyProfiles.length} profiles found in storage`); } catch (_) { }
        ...
        if (verifyProfiles.length !== profiles.length) {
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log(`[Bulk Modal] ✅ VERIFICATION SUCCESS: All ${verifyProfiles.length} profiles persisted correctly`); } catch (_) { }
        }
    ...
    $(document).on('click', '#bulk-apply-btn', async function () {
        ...
        try {
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log('[Bulk Modal] 🔄 Flushing changes to IndexedDB before closing modal...'); } catch (_) { }
            try {
                if (window.__IDB_FLUSH_PENDING__) {
                    await window.__IDB_FLUSH_PENDING__();
                    try { if (window.SCAN_LOG_ENABLED) console.log('[Bulk Modal] ✅ All changes persisted to IndexedDB'); } catch (_) { }
                }
            } catch (e) {
