...
const CONFIG_APP = {
    APP: {
        ...
        VOL_CHECK: true,
        META_DEX: false,     // Set true untuk menghidupkan fitur META-DEX aggregator (LIFI, Rubic, Rango, Kamino)
    }
...
const CONFIG_DEXS = {
    ...
    // =============================
    GLOBAL: {
        // ✅ Fallback completely removed - pure rotation only
        allowFallback: false,

        // ✅ Global timeout configuration (Option C: Per-provider with wildcard)
        timeout: {
            // ========== Official APIs ==========
            // ⚡ Optimized for fast scanning (max 5s to avoid long delays with 500+ coins)
            'velora6': 4000,         // Velora v6.2 (api.paraswap.io): 4s
            'velora5': 4000,         // Velora v5 (apiv5.paraswap.io): 4s - load distribution
            'kyber': 3000,           // KyberSwap: 3s (fast API)
            'odos': 3500,            // ODOS: 3.5s
            'odos2': 3500,           // ODOS v2: 3.5s
            'odos3': 3500,           // ODOS v3: 3.5s
            'hinkal-odos': 3500,     // Hinkal ODOS Proxy: 3.5s (privacy-focused, typically faster)
            'okx': 3500,             // OKX DEX: 3.5s
            'relay': 4000,           // Relay: 4s
            'flytrade': 4000,        // Flytrade: 4s
            'jupiter': 3000,         // Jupiter: 3s (Solana fast)
            'dflow': 3000,           // DFlow: 3s (Solana fast)
            'matcha': 3500,          // Matcha/0x: 3.5s

            // ========== Meta-Aggregators (Wildcard) ==========
            // Aggregators need slightly more time but still under 5s
            'lifi-*': 5000,          // All LIFI filtered: 5s (max)
            'swoop-*': 8000,         // All SWOOP filtered: 8s (railway.app slower, prevent cancel)
            'swing-*': 5000,         // All SWING filtered: 5s (max)
            'dzap-*': 5000,          // All DZAP filtered: 5s (max) - WARNING: 429 rate limit issues
            'rango-*': 5000,         // All Rango filtered: 5s (max) - WARNING: 403 forbidden issues
            'rubic-*': 5000,         // All Rubic filtered: 5s (max)

            // ========== Multi-DEX Aggregators ==========
            // Multi-quote aggregators - slightly longer but capped at 8s
            'lifi': 5000,            // LIFI multi-quote: 5s
            'swoop': 8000,           // SWOOP multi-quote: 8s (railway.app needs more time)
            'swing': 5000,           // SWING multi-quote: 5s
            'dzap': 5000,            // DZAP multi-quote: 5s - WARNING: 429 rate limit issues
            'rango': 5000,           // Rango multi-quote: 5s - WARNING: 403 forbidden issues
            'rubic': 5000,           // Rubic multi-quote: 5s
            'kamino': 5000,          // Kamino (Solana): 5s

            // ========== Default Fallback ==========
            'default': 4000          // Default: 4s (balanced)
        }
    },

    // =============================
    // PER-DEX CONFIGURATION
    // =============================
    kyber: {
        label: 'KyberSwap',
        ...
        // ✅ allowFallback removed - managed globally (GLOBAL.allowFallback: false)
        // ✅ timeout removed - managed globally (GLOBAL.timeout)
    },
    sushi: {
        label: 'SUSHI',
        badgeClass: 'bg-sushi',
        ...
        // ✅ timeout removed - managed globally
    },
    okx: {
        label: 'OKXDEX',
        badgeClass: 'bg-okxdex',
        disabled: false, // ✅ ENABLED - OKX DEX Aggregator active
        solanaSupport: true, // ✅ Supports Solana chain
        warna: "#000000",
    ...
    matcha: {
        ...
        //
        // PRIMARY ROTATION:
        // - tokentopair (CEX→DEX): delta-matcha = 1Delta proxy (fast, free, no API key needed)
        // - pairtotoken (DEX→CEX): matcha = Direct 0x API (requires API key)
        //
        // SECONDARY ROTATION (if primary fails):
        // - tokentopair: swoop-matcha = SWOOP aggregator (slower but reliable)
        // - pairtotoken: swoop-matcha = SWOOP aggregator (backup)
        //
        // ⚠️ SWOOP NOTES:
        // - Endpoint: https://bzvwrjfhuefn.up.railway.app/swap
        // - Timeout: 8000ms (longer due to railway.app latency)
        // - Browser may cancel if too many concurrent requests
        // - Delay: 200ms between SWOOP requests recommended
        //
        // ❌ AVOID: rango-matcha (403 Forbidden) and dzap-matcha (429 Rate Limit)
        fetchdex: {
            primary: {
    ...
    odos: {
        ...
        //
        // PRIMARY ROTATION:
        // - tokentopair (CEX→DEX): odos3 = Official ODOS v3 API (direct)
        // - pairtotoken (DEX→CEX): hinkal-odos = Hinkal ODOS Proxy (privacy + speed)
        //
        //   Hinkal-ODOS Proxy Benefits:
        //   ✅ 1-2 seconds faster than direct ODOS API
        //   ✅ Privacy-focused (no direct IP exposure to ODOS)
        //   ✅ Same response format as official ODOS
        //   ✅ Timeout: 3.5s (same as odos3)
        ...
        // ✅ allowFallback removed - managed globally
        // ✅ timeout removed - managed globally (hinkal-odos: 3500ms)
    },
    // ============ DISABLED DEXes ============
    ...
    velora: {
        ...
        // ⚡ ROTATION STRATEGY: Alternate between official API and filtered aggregators
        // ⚡ LOAD DISTRIBUTION: Alternate between v6 and v5 APIs
        // - v6 (api.paraswap.io): tokentopair direction
        // - v5 (apiv5.paraswap.io): pairtotoken direction  
        // This distributes RPS load across different ParaSwap infrastructure
        fetchdex: {
            primary: {
                tokentopair: 'velora6',        // CEX→DEX: Official Velora v6.2 (api.paraswap.io)
                pairtotoken: 'velora5'         // DEX→CEX: Velora v5 (apiv5.paraswap.io) - load distribution
            },
            secondary: {
                tokentopair: 'swing-velora',        // CEX→DEX: Fallback to v5
                pairtotoken: 'swoop-velora'         // DEX→CEX: Fallback to v6
            }
        }
        // ✅ allowFallback removed - managed globally
        // ✅ timeout removed - managed globally
    ...
    jupiter: {
        ...
        solanaSupport: true, // ✅ Solana-only DEX
        ...
        // ✅ NEW: Timeout per strategy (ms)
        timeout: {
            'jupiter': 1000    // Jupiter API (Solana): 10 detik
        }
    },
    ...
    dflow: {
        ...
        // ✅ NEW: Timeout per strategy (ms)
        timeout: {
            'dflow': 1200      // DFlow API (Solana): 12 detik
        }
    },
    ...
    kamino: {
        ...
        proxy: true,
        solanaSupport: true, // ✅ Solana-only DEX aggregator
        warna: "#7c3aed", // Kamino purple
        isMetaDex: true,  // ✅ META-DEX: Mark as meta-aggregator (Solana only)
        isMultiDex: true, // ⭐ Multi-quote aggregator (returns top 3 routes from multiple DEXes)
        builder: ({ tokenAddress, pairAddress }) =>
        ...
        // ✅ NEW: Timeout per strategy (ms)
        timeout: {
            'kamino': 1500     // Kamino multi-aggregator (Solana): 25 detik (banyak routes)
        }
    },
    ...
    lifi: {
        ...
        // ✅ NEW: Timeout per strategy (ms)
        timeout: {
            'lifi': 800     // LIFI REST API provider: 18 detik
        }
    },
    ...
    rubic: {
        ...
        // ✅ NEW: Timeout per strategy (ms)
        timeout: {
            'rubic': 2500    // Rubic multi-aggregator: 25 detik (banyak providers)
        }
    },
    ...
    rango: {
        ...
        // ✅ NEW: Timeout per strategy (ms)
        timeout: {
            'rango': 2200    // Rango multi-aggregator: 22 detik
        }
    },
