...
(function (global) {
    ...
    function initializeDBConfig() {
        try {
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log('[Database Viewer] Initialized with config:', DB_CONFIG); } catch(_) {}
            try { if (window.SCAN_LOG_ENABLED) console.log('[Database Viewer] Expected stores: APP_KV_STORE, SNAPSHOT_STORE'); } catch(_) {}
        } catch (err) {
    ...
    function getAllChainKeys() {
        try {
            ...
            if (chainKeys.length > 0) {
                try { if (window.SCAN_LOG_ENABLED) console.log('[Database Viewer] Chains from CONFIG_CHAINS:', chainKeys); } catch(_) {}
                return chainKeys;
            }

            // Fallback: Detect from IndexedDB keys
            try { if (window.SCAN_LOG_ENABLED) console.warn('[Database Viewer] CONFIG_CHAINS not found, using fallback chain list'); } catch(_) {}
    ...
    async function loadAllTableData() {
        ...
        try { if (window.SCAN_LOG_ENABLED) console.log('[Database Viewer] Loading tables with config:', DB_CONFIG); } catch(_) {}
        ...
        try {
            if (typeof window.exportIDB === 'function') {
                ...
                // ✅ NEW: CEX API Keys
                const cexKeys = getKv('CEX_API_KEYS');
                if (cexKeys && typeof cexKeys === 'object' && Object.keys(cexKeys).length > 0) {
                    tables['CEX_API_KEYS'] = {
                        name: 'CEX_API_KEYS',
                        displayName: 'CEX API Keys',
                        type: 'cex_keys',
                        rawKey: (kvIndex.get('CEX_API_KEYS') || {}).rawKey || 'CEX_API_KEYS',
                        data: cexKeys,
                        count: Object.keys(cexKeys).length
                    };
                }
            ...
            try { if (window.SCAN_LOG_ENABLED) console.warn('[Database Viewer] Storage module path failed, falling back to direct IDB:', e); } catch(_) {}
        }
        ...
        try {
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log('[Database Viewer] All keys in APP_KV_STORE:', allKeys); } catch(_) {}

            // 1. Load SETTING_SCANNER
            try { if (window.SCAN_LOG_ENABLED) console.log('[Database Viewer] Loading SETTING_SCANNER...'); } catch(_) {}
            const settings = await getFromDB(db, DB_CONFIG.store, 'SETTING_SCANNER');
            try { if (window.SCAN_LOG_ENABLED) console.log('[Database Viewer] SETTING_SCANNER result:', settings); } catch(_) {}
            if (settings) {
                tables['SETTING_SCANNER'] = {
            ...
            const chains = getAllChainKeys();
            try { if (window.SCAN_LOG_ENABLED) console.log('[Database Viewer] Chains found:', chains); } catch(_) {}
            ...
            for (const chain of chains) {
                ...
                const key = `TOKEN_${chainUpper}`;
                try { if (window.SCAN_LOG_ENABLED) console.log(`[Database Viewer] Loading ${key}...`); } catch(_) {}
                const koinData = await getFromDB(db, DB_CONFIG.store, key);
                try { if (window.SCAN_LOG_ENABLED) console.log(`[Database Viewer] ${key} result:`, koinData ? `${koinData.length} items` : 'null'); } catch(_) {}
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log('[Database Viewer] Loading TOKEN_MULTICHAIN...'); } catch(_) {}
            const multiTokens = await getFromDB(db, DB_CONFIG.store, 'TOKEN_MULTICHAIN');
            try { if (window.SCAN_LOG_ENABLED) console.log('[Database Viewer] TOKEN_MULTICHAIN result:', multiTokens ? `${multiTokens.length || 0} items` : 'null'); } catch(_) {}
            ...
            for (const chain of chains) {
                ...
                try { if (window.SCAN_LOG_ENABLED) console.log(`[Database Viewer] ${key} result:`, filterData ? 'Found' : 'null'); } catch(_) {}
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log('[Database Viewer] Loading FILTER_MULTICHAIN...'); } catch(_) {}
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log('[Database Viewer] FILTER_MULTICHAIN result:', filterMulti); } catch(_) {}
            if (filterMulti) {
            ...
            // ✅ NEW: Load CEX_API_KEYS
            try { if (window.SCAN_LOG_ENABLED) console.log('[Database Viewer] Loading CEX_API_KEYS...'); } catch(_) {}
            let cexApiKeys = await getFromDB(db, DB_CONFIG.store, 'CEX_API_KEYS');
            if (!cexApiKeys && typeof window.getFromLocalStorage === 'function') {
                cexApiKeys = window.getFromLocalStorage('CEX_API_KEYS', undefined);
            }
            try { if (window.SCAN_LOG_ENABLED) console.log('[Database Viewer] CEX_API_KEYS result:', cexApiKeys); } catch(_) {}
            if (cexApiKeys && typeof cexApiKeys === 'object') {
                tables['CEX_API_KEYS'] = {
                    name: 'CEX_API_KEYS',
                    displayName: 'CEX API Keys',
                    type: 'cex_keys',
                    data: cexApiKeys,
                    count: Object.keys(cexApiKeys).length
                };
            }
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log('[Database Viewer] ✅ Total tables loaded:', Object.keys(tables).length); } catch(_) {}
            try { if (window.SCAN_LOG_ENABLED) console.log('[Database Viewer] Table list:', Object.keys(tables)); } catch(_) {}
        ...
        return new Promise((resolve) => {
            try {
                ...
                request.onsuccess = () => {
                    ...
                    if (!result) {
                        try { if (window.SCAN_LOG_ENABLED) console.log(`[Database Viewer] ${storeName}/${key}: No data found`); } catch(_) {}
                        resolve(null);
                        return;
                    }
                    ...
                    if (result.val !== undefined) {
                        ...
                        try { if (window.SCAN_LOG_ENABLED) console.log(`[Database Viewer] ${storeName}/${key}: ✅ Found - Type: ${dataType}`); } catch(_) {}
                        resolve(result.val);
                    } else if (result.value !== undefined) {
                        try { if (window.SCAN_LOG_ENABLED) console.log(`[Database Viewer] ${storeName}/${key}: Found with .value format`); } catch(_) {}
                        resolve(result.value);
                    } else {
                        try { if (window.SCAN_LOG_ENABLED) console.log(`[Database Viewer] ${storeName}/${key}: Found with direct format`); } catch(_) {}
                        resolve(result);
                    }
                };
    ...
    function renderTableSummary(table) {
        if (table.type === 'snapshot') {
            ...
            `;
        } else if (table.type === 'cex_keys') {
            return `
                <div class="db-table-summary">
                    <span class="summary-item">
                        <strong>${table.count}</strong> CEX terkonfigurasi
                    </span>
                </div>
            `;
        }
    ...
    function renderTableData(table) {
        if (table.type === 'snapshot') {
            ...
            return renderCEXKeysData(table.data);
        }
    ...
    /**
     * ✅ NEW: Render CEX API Keys data
     * Shows CEX names with masked API keys for security
     */
    function renderCEXKeysData(data) {
        if (!data || typeof data !== 'object' || Object.keys(data).length === 0) {
            return '<p class="uk-text-muted uk-text-center">Tidak ada CEX API key yang dikonfigurasi</p>';
        }
            ...
            <div class="uk-overflow-auto">
                <table class="uk-table uk-table-divider uk-table-hover uk-table-small db-data-table">
                    <thead>
                        <tr>
                            <th style="width:40px">No</th>
                            <th style="width:120px">CEX</th>
                            <th>API Key</th>
                            <th>Secret Key</th>
                            <th style="width:100px">Passphrase</th>
                        </tr>
                    </thead>
                    <tbody>
        `;

        Object.keys(data).forEach((cex, idx) => {
            const cexData = data[cex];
            const color = cexColors[cex] || '#667eea';
            const hasPassphrase = cexData.Passphrase ? true : false;

            html += `
                <tr>
                    <td class="uk-text-muted">${idx + 1}</td>
                    <td class="uk-text-bold" style="color: ${color};">${cex}</td>
                    <td><code class="uk-text-small">${maskKey(cexData.ApiKey)}</code></td>
                    <td><code class="uk-text-small">${maskKey(cexData.ApiSecret)}</code></td>
                    <td class="uk-text-center">
                        ${hasPassphrase ? '<span style="color:#28a745" title="Passphrase configured">✓</span>' : '<span class="uk-text-muted">-</span>'}
                    </td>
                </tr>
            `;
        });
        ...
        html += `
            ...
            <div class="uk-alert uk-alert-primary uk-margin-small-top">
                <p class="uk-text-small uk-margin-remove">
                    ...
                    <strong>Catatan:</strong> API keys ditampilkan dengan masking untuk keamanan. 
                    Data lengkap tetap tersimpan di IndexedDB dan dapat di-export/import.
                </p>
            </div>
        `;
    ...
    function renderDatabaseView() {
        ...
        const grouped = {
            ...
            filter: [],
            modal_profile: [],
            cex_keys: []  // ✅ NEW: Added for CEX API Keys
        };
        ...
        tables.forEach(table => {
            if (grouped[table.type]) {
                ...
                try { if (window.SCAN_LOG_ENABLED) console.warn('[Database Viewer] Unknown table type:', table.type, table.name); } catch(_) {}
            }
        });
        ...
        // ✅ NEW: Render CEX API Keys
        if (grouped.cex_keys.length > 0) {
            html += renderTableGroup('CEX API Keys', grouped.cex_keys);
        }
    ...
    function init() {
        ...
        try { if (window.SCAN_LOG_ENABLED) console.log('[Database Viewer] Module initialized'); } catch(_) {}
    }
