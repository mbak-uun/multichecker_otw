...
(function initCEXService(global) {
    ...
    // ✅ FIX: Queue for failed CEX requests (due to tab inactive throttling)
    const pendingCEXRequests = [];
    root.__CEX_PENDING_REQUESTS__ = pendingCEXRequests;
    ...
    function getPriceCEX(coins, NameToken, NamePair, cex, tableBodyId) {
        return new Promise((resolve, reject) => {
            // ✅ FIX: Store request params for potential retry if tab was inactive
            const requestParams = { coins, NameToken, NamePair, cex, tableBodyId };
            ...
            Promise.all(promises).then(resultsArray => {
                ...
                // ✅ FIX: If tab was hidden during request, queue for retry when tab becomes visible
                if (typeof document !== 'undefined' && document.hidden) {
                    try { if (window.SCAN_LOG_ENABLED) console.warn(`[CEX] ⚠️ Request failed while tab inactive: ${cex} ${NameToken}/${NamePair}`);
                    } catch(_) {}
                    pendingCEXRequests.push(requestParams);
                }
                reject(error);
            });
        });
    ...
    async function fetchWalletStatus(cex) {
        // ✅ NEW: Get API keys from IndexedDB via getCEXCredentials()
        let ApiKey, ApiSecret, Passphrase;

        if (typeof getCEXCredentials === 'function') {
            const credentials = await getCEXCredentials(cex);
            if (!credentials) {
                throw new Error(`${cex} API Key/Secret not configured. Please configure in Settings.`);
            }
            ApiKey = credentials.ApiKey;
            ApiSecret = credentials.ApiSecret;
            Passphrase = credentials.Passphrase; // For KUCOIN and BITGET
        } else {
            // Fallback to old method (should not happen)
            try { if (window.SCAN_LOG_ENABLED) console.warn(`[fetchWalletStatus] getCEXCredentials not available, using fallback`);
            } catch(_) {}
        ...
        switch (cex) {
            ...
            case 'LBANK': {
                ...
                try { if (window.SCAN_LOG_ENABLED) console.log(`[LBANK] ✅ Fetched ${arr.length} coins from assetConfigs endpoint (includes deposit & withdraw status)`);
                } catch(_) {}
                return arr;
            }
    ...
    function applyWalletStatusToTokenList(tokenListName, walletStatusMap, options) {
        try { if (window.SCAN_LOG_ENABLED) console.log('🔧 [applyWalletStatusToTokenList] FUNCTION CALLED for:', tokenListName);
        } catch(_) {}

        const opts = options || {};
        const allWalletStatus = walletStatusMap || getFromLocalStorage('CEX_WALLET_STATUS', {});

        try { if (window.SCAN_LOG_ENABLED) console.log('🔧 [applyWalletStatusToTokenList] CEX count:', Object.keys(allWalletStatus).length);
        } catch(_) {}
        try { if (window.SCAN_LOG_ENABLED) console.log('🔧 [applyWalletStatusToTokenList] Sample CEX data:', Object.keys(allWalletStatus)[0], allWalletStatus[Object.keys(allWalletStatus)[0]]);
        } catch(_) {}
        ...
        const updatedTokens = tokens.map(token => {
            ...
            (token.selectedCexs || Object.keys(CONFIG_CEX)).forEach(cexKey => {
                ...
                const updateForSymbol = (symbol, isTokenIn) => {
                    ...
                    if (!walletInfo) {
                        try { if (window.SCAN_LOG_ENABLED) console.warn(`[applyWallet] ${cexKey} ${symbolUpper}: walletInfo tidak ada (token tidak listing di CEX ini)`);
                        } catch(_) {}
                        return;
                    }
                    ...
                    try { if (window.SCAN_LOG_ENABLED) console.log(`[applyWallet] ${cexKey} ${symbolUpper} (isToken=${isTokenIn}):`);
                    } catch(_) {}
                    try { if (window.SCAN_LOG_ENABLED) console.log(`[applyWallet]   chainLabel=${chainLabelForCEX}`);
                    } catch(_) {}
                    try { if (window.SCAN_LOG_ENABLED) console.log(`[applyWallet]   walletInfo keys:`, Object.keys(walletInfo));
                    } catch(_) {}
                    try { if (window.SCAN_LOG_ENABLED) console.log(`[applyWallet]   match:`, match);
                    } catch(_) {}
                        ...
                        if (availableChains.length > 0) {
                            ...
                            for (const variation of chainVariations) {
                                if (walletInfo[variation]) {
                                    ...
                                    try { if (window.SCAN_LOG_ENABLED) console.log(`[applyWallet]   ✅ FALLBACK MATCH dengan "${variation}"`);
                                    } catch(_) {}
                                    break;
                                }
                            }
                            ...
                            if (!match && availableChains.length > 0) {
                                ...
                                try { if (window.SCAN_LOG_ENABLED) console.warn(`[applyWallet]   ⚠️ USING FIRST AVAILABLE CHAIN: ${availableChains[0]}`);
                                } catch(_) {}
                            }
                        }
                    }
                    ...
                    if (match) {
                        ...
                        try { if (window.SCAN_LOG_ENABLED) console.log(`[applyWallet]   ✅ SAVED ${depositField}=${!!match.depositEnable}, ${withdrawField}=${!!match.withdrawEnable}, feeWD=${match.feeWDs}`);
                        } catch(_) {}
                    } else {
        ...
        if (updatedTokens.length > 0) {
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log(`💾 [applyWallet] Sample saved token:`, sample.symbol_in, '⇄', sample.symbol_out);
            } catch(_) {}
            try { if (window.SCAN_LOG_ENABLED) console.log(`💾 [applyWallet] Sample dataCexs:`, sample.dataCexs);
            } catch(_) {}
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log(`💾 [applyWallet] Tokens with CEX data: ${validCount}, without: ${invalidCount}`);
            } catch(_) {}
        }
    ...
    async function checkAllCEXWallets() {
        try { if (window.SCAN_LOG_ENABLED) console.log('');
        } catch(_) {}
        try { if (window.SCAN_LOG_ENABLED) console.log('='.repeat(80));
        } catch(_) {}
        try { if (window.SCAN_LOG_ENABLED) console.log('🚀 [checkAllCEXWallets] FUNCTION CALLED - Starting wallet update...');
        } catch(_) {}
        try { if (window.SCAN_LOG_ENABLED) console.log('='.repeat(80));
        } catch(_) {}
        try { if (window.SCAN_LOG_ENABLED) console.log('');
        ...
        try { if (window.SCAN_LOG_ENABLED) console.log(`📊 [checkAllCEXWallets] Build wallet status map: ${itemCount} items dari ${aggregated.flat().length} total`);
        } catch(_) {}
        try { if (window.SCAN_LOG_ENABLED) console.log(`📊 [checkAllCEXWallets] CEX count: ${Object.keys(walletStatusByCex).length}`);
        } catch(_) {}
        if (Object.keys(walletStatusByCex).length > 0) {
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log(`📊 [checkAllCEXWallets] Sample ${firstCex}: ${tokenCount} tokens`);
            } catch(_) {}
            if (tokenCount > 0) {
                const firstToken = Object.keys(walletStatusByCex[firstCex])[0];
                const chains = Object.keys(walletStatusByCex[firstCex][firstToken] || {});
                try { if (window.SCAN_LOG_ENABLED) console.log(`📊 [checkAllCEXWallets]   Sample token ${firstToken}: chains = [${chains.join(', ')}]`);
                } catch(_) {}
                if (chains.length > 0) {
                    const sampleData = walletStatusByCex[firstCex][firstToken][chains[0]];
                    try { if (window.SCAN_LOG_ENABLED) console.log(`📊 [checkAllCEXWallets]   Sample data:`, sampleData);
                    } catch(_) {}
                }
            }
        }
        ...
        try {
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log('💾 [checkAllCEXWallets] Data saved to CEX_WALLET_STATUS');
            } catch(_) {}
            try { if (window.SCAN_LOG_ENABLED) console.log('💾 [checkAllCEXWallets] Structure:', {
        ...
        try {
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log('📦 [checkAllCEXWallets] Applying wallet status to token stores:', Array.from(tokenStores));
            } catch(_) {}
            try { if (window.SCAN_LOG_ENABLED) console.log('📦 [checkAllCEXWallets] walletStatusByCex keys:', Object.keys(walletStatusByCex));
            } catch(_) {}

            tokenStores.forEach(storeKey => {
                const quiet = storeKey !== activeKey;
                try { if (window.SCAN_LOG_ENABLED) console.log(`📝 [checkAllCEXWallets] Calling applyWalletStatusToTokenList for ${storeKey}...`);
                } catch(_) {}
                try {
    ...
    async function fetchAllCEXPrices(cex) {
        ...
        try { if (window.SCAN_LOG_ENABLED) console.log(`[fetchAllCEXPrices] Fetching ALL prices from ${cexUpper}...`);
        } catch(_) {}
        ...
        try {
            ...
            try { if (window.SCAN_LOG_ENABLED) console.warn('[fetchAllCEXPrices] Failed to load timeout setting, using default:', e.message);
            } catch(_) {}
        }
        ...
        try {
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log(`[fetchAllCEXPrices] ✅ ${cexUpper}: Fetched ${count} prices`);
            } catch(_) {}
    ...
    // ✅ FIX: Re-populate empty CEX orderbook cells when tab becomes visible
    // Solves issue where browser throttles AJAX requests when tab is inactive
    if (typeof document !== 'undefined' && !root.__CEX_VISIBILITY_HANDLER_SET__) {
        document.addEventListener('visibilitychange', function () {
            if (!document.hidden) {
                // Tab is now visible - check for empty orderbook cells and retry pending requests
                setTimeout(function () {
                    try {
                        const emptyCells = Array.from(document.querySelectorAll('.td-orderbook')).filter(cell => {
                            const content = cell.textContent.trim();
                            const hasHTML = cell.innerHTML.trim();
                            // Cell is empty if it has no text content and no meaningful HTML
                            return !content && (!hasHTML || hasHTML === '&nbsp;');
                        });

                        if (emptyCells.length > 0) {
                            try { if (window.SCAN_LOG_ENABLED) console.log(`[CEX] 🔄 Tab became visible - Found ${emptyCells.length} empty orderbook cells`);
                            } catch(_) {}
                        }

                        // ✅ FIX: Retry pending CEX requests that failed due to tab being inactive
                        if (pendingCEXRequests.length > 0) {
                            const toRetry = pendingCEXRequests.splice(0); // Clear queue and get all items
                            try { if (window.SCAN_LOG_ENABLED) console.log(`[CEX] ⚡ Auto-retrying ${toRetry.length} failed CEX requests...`);
                            } catch(_) {}

                            // Retry with delay to avoid overwhelming the API
                            toRetry.forEach((params, index) => {
                                setTimeout(() => {
                                    getPriceCEX(params.coins, params.NameToken, params.NamePair, params.cex, params.tableBodyId)
                                        .then(() => {
                                            try { if (window.SCAN_LOG_ENABLED) console.log(`[CEX] ✅ Retry successful: ${params.cex} ${params.NameToken}/${params.NamePair}`);
                                            } catch(_) {}
                                        })
                                        .catch(err => {
                                            try { if (window.SCAN_LOG_ENABLED) console.warn(`[CEX] ❌ Retry failed: ${params.cex} ${params.NameToken}/${params.NamePair}`, err);
                                            } catch(_) {}
                                        });
                                }, index * 200); // Stagger requests by 200ms each
                            });
                        } else if (emptyCells.length > 0) {
                            try { if (window.SCAN_LOG_ENABLED) console.log('[CEX] 💡 Tip: Orderbook cells are empty due to browser throttling inactive tabs');
                            } catch(_) {}
                            try { if (window.SCAN_LOG_ENABLED) console.log('[CEX] 💡 Workaround: Keep tab active during scanning, or re-run scan');
                            } catch(_) {}
                        }
                    } catch (e) {
                        try { if (window.SCAN_LOG_ENABLED) console.warn('[CEX] Visibility handler error:', e);
                        } catch(_) {}
                    }
                }, 500); // Delay to ensure DOM is ready
            }
        });
        root.__CEX_VISIBILITY_HANDLER_SET__ = true;
        try { if (window.SCAN_LOG_ENABLED) console.log('[CEX] ✅ Visibility change handler installed');
        } catch(_) {}
    }
