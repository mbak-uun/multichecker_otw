...
(function initDEXService(global) {
  ...
  function get0xApiKey() {
    try {
      ...
      try { if (window.SCAN_LOG_ENABLED) console.warn('[0x API] get0xApiKey() from secrets.js not available'); } catch (_) { }
      return null;
    } catch (error) {
  ...
  const dexStrategies = {
    ...
    /**
     * Velora v5 API - Alternative endpoint for load distribution
     * Endpoint: https://apiv5.paraswap.io/prices/
     * 
     * Differences from v6:
     * - Uses apiv5.paraswap.io instead of api.paraswap.io
     * - Excludes ParaSwapPool and ParaSwapLimitOrders (reduces overhead)
     * - Uses 'llamaswap' as partner identifier
     * 
     * Benefits:
     * - Distributes API load across different infrastructure
     * - Reduces rate limiting risk when combined with velora6
     */
    velora5: {
      buildRequest: ({ codeChain, sc_input, sc_output, amount_in_big, des_input, des_output, SavedSettingData }) => {
        const userAddr = SavedSettingData?.walletMeta || '0x0000000000000000000000000000000000000000';
      ...
      parseResponse: (response, { des_output, chainName }) => {
        ...
        return {
          ...
          FeeSwap,
          dexTitle: 'VELORA',
          routeTool: 'VELORA V5'  // Track that it came via v5 API
        };
      }
    },
    'hinkal-odos': {
    ...
    matcha: {
      buildRequest: ({ chainName, sc_input_in, sc_output_in, amount_in_big, codeChain, sc_output, sc_input, SavedSettingData }) => {
        ...
        try { if (window.SCAN_LOG_ENABLED) console.log(`[Matcha API] Request: ${chainName} ${sc_input_in} -> ${sc_output_in}`); } catch (_) { }
      ...
      parseResponse: (response, { des_output, des_input, chainName }) => {
        ...
        try {
          ...
          try { if (window.SCAN_LOG_ENABLED) console.warn('[0x API] Could not parse gas fee from response, using default'); } catch (_) { }
        }

        // Log response details for debugging
        try {
          if (window.SCAN_LOG_ENABLED) console.log(`[Matcha API] Response parsed:`, {
        ...
        if (response.issues) {
          ...
          if (issueKeys.length > 0) {
            try { if (window.SCAN_LOG_ENABLED) console.warn(`[0x API] Response issues:`, issueKeys.join(', ')); } catch (_) { }
          }
        }
        ...
        if (response.route?.fills) {
          ...
          if (sources.length > 0) {
            try { if (window.SCAN_LOG_ENABLED) console.log(`[Matcha API] Liquidity sources:`, sources.join(', ')); } catch (_) { }
          }
        }
    ...
    'delta-matcha': {
      buildRequest: ({ chainName, sc_input_in, sc_output_in, amount_in_big, codeChain, SavedSettingData }) => {
        ...
        try { if (window.SCAN_LOG_ENABLED) console.log(`[1Delta Matcha] Request: ${chainName} ${sc_input_in} -> ${sc_output_in}`); } catch (_) { }
      ...
      parseResponse: (response, { des_output, chainName }) => {
        ...
        try {
          ...
          try { if (window.SCAN_LOG_ENABLED) console.warn('[1Delta] Could not parse gas fee, using default'); } catch (_) { }
        }

        try {
          if (window.SCAN_LOG_ENABLED) console.log(`[1Delta Matcha] Response parsed:`, {
            buyAmount: response.buyAmount,
            amountOut: amount_out.toFixed(6),
    ...
    sushi: {
      buildRequest: ({ codeChain, sc_input_in, sc_output_in, amount_in_big, SavedSettingData }) => {
        ...
        try { if (window.SCAN_LOG_ENABLED) console.log(`[Sushi API] Request: Chain ${codeChain} ${sc_input_in} -> ${sc_output_in}`); } catch (_) { }
      ...
      parseResponse: (response, { des_output, chainName }) => {
        ...
        if (response.route && response.route.legs) {
          try { if (window.SCAN_LOG_ENABLED) console.log(`[Sushi] Route has ${response.route.legs.length} legs, price impact: ${response.priceImpact || 'N/A'}%`); } catch (_) { }
        }
    ...
    flytrade: {
      buildRequest: ({ codeChain, chainName, sc_input_in, sc_output_in, amount_in_big, SavedSettingData }) => {
        ...
        try { if (window.SCAN_LOG_ENABLED) console.log(`[Flytrade API] Request: ${network} ${sc_input_in} -> ${sc_output_in}`); } catch (_) { }
      ...
      parseResponse: (response, { des_output, chainName }) => {
        ...
        try {
          ...
          try { if (window.SCAN_LOG_ENABLED) console.warn('[Flytrade] Could not parse gas fee, using default'); } catch (_) { }
        }
  ...
  dexStrategies.dzap = {
    ...
    parseResponse: (response, { des_output, chainName }) => {
      ...
      try { if (window.SCAN_LOG_ENABLED) console.log(`[DZAP] Returning best provider: ${bestResult.amount_out.toFixed(6)} (from ${Object.keys(quoteRates).length} providers)`); } catch (_) { }
  ...
  dexStrategies.lifi = {
    ...
    parseResponse: (response, { des_output, chainName }) => {
      ...
      try { if (window.SCAN_LOG_ENABLED) console.log(`[LIFI] Returning best route: ${amount_out.toFixed(6)} (from ${routes.length} available routes)`); } catch (_) { }
  ...
  dexStrategies['lifi-odos'] = {
    ...
    parseResponse: (response, { des_output, chainName }) => {
      ...
      try { if (window.SCAN_LOG_ENABLED) console.log(`[LIFI-ODOS] Using ODOS via LIFI: ${amount_out.toFixed(6)} output, gas: $${FeeSwap.toFixed(4)}`); } catch (_) { }
  ...
  function createFilteredLifiStrategy(dexKey, dexTitle) {
    return {
      ...
      parseResponse: (response, { des_output, chainName }) => {
        ...
        try { if (window.SCAN_LOG_ENABLED) console.log(`[LIFI-${dexTitle}] Using ${dexTitle} via LIFI: ${amount_out.toFixed(6)} output, gas: $${FeeSwap.toFixed(4)}`); } catch (_) { }
  ...
  function createFilteredLifiRelayStrategy() {
    return {
      ...
      parseResponse: (response, { des_output, chainName }) => {
        ...
        try { if (window.SCAN_LOG_ENABLED) console.log(`[LIFI-RELAY] Using optimized LIFI routing: ${amount_out.toFixed(6)} output, gas: $${FeeSwap.toFixed(4)}`); } catch (_) { }
  ...
  function createFilteredSwoopStrategy(aggregatorSlug, dexTitle) {
    return {
      ...
      parseResponse: (response, { des_output, chainName }) => {
        ...
        try { if (window.SCAN_LOG_ENABLED) console.log(`[SWOOP-${dexTitle}] Using ${dexTitle} via SWOOP: ${amount_out.toFixed(6)} output`); } catch (_) { }
  ...
  function createFilteredDzapStrategy(dexKey, dexTitle) {
    return {
      ...
      parseResponse: (response, { des_output, chainName }) => {
        ...
        try { if (window.SCAN_LOG_ENABLED) console.log(`[DZAP-${dexTitle}] Using ${dexTitle} via DZAP: ${amount_out.toFixed(6)} output, gas: $${FeeSwap.toFixed(4)}`); } catch (_) { }
   ...
   * This converts SWING from multi-aggregator into a filtered REST API provider
   * Similar to filtered LIFI/SWOOP/DZAP
   * 
   * @param {string} providerFilter - Provider name to filter (e.g., 'paraswap', 'uniswap')
   * @param {string} dexTitle - Display name for the DEX (e.g., 'VELORA', 'UNISWAP')
   */
  function createFilteredSwingStrategy(providerFilter, dexTitle) {
    return {
      buildRequest: ({ codeChain, sc_input, sc_output, amount_in_big }) => {
        ...
        if (!chainSlug) {
          throw new Error(`Swing does not support chain ID ${codeChain}`);
        }
        ...
        if (wrappedNativeAddresses[fromToken]) fromToken = wrappedNativeAddresses[fromToken];
        if (wrappedNativeAddresses[toToken]) toToken = wrappedNativeAddresses[toToken];
        ...
        // Get project ID from secrets.js
        ...
        try {
          ...
          try { if (window.SCAN_LOG_ENABLED) console.warn('[SWING] Failed to get projectId from secrets.js, using default:', e.message); } catch (_) { }
        }

        try { if (window.SCAN_LOG_ENABLED) console.log(`[SWING-${dexTitle}] Using projectId: ${selectedProjectId} (1 of ${totalProjects} projects)`); } catch (_) { }
      ...
      parseResponse: (response, { des_output, chainName }) => {
        ...
        try { if (window.SCAN_LOG_ENABLED) console.log(`[SWING-${dexTitle}] Using ${dexTitle} via SWING: ${amount_out.toFixed(6)} output, gas: $${FeeSwap.toFixed(4)}`); } catch (_) { }
  ...
  dexStrategies['swing-velora'] = createFilteredSwingStrategy('velora', 'VELORA');  // ✅ Swing uses 'velora-delta' not 'paraswap'
  ...
  dexStrategies.swing = {
    buildRequest: ({ codeChain, sc_input, sc_output, amount_in_big, sc_input_in, sc_output_in }) => {
      ...
      try { if (window.SCAN_LOG_ENABLED) console.log(`[SWING] Using projectId: ${selectedProjectId} (1 of ${totalProjects} projects)`); } catch (_) { }
    ...
    parseResponse: (response, { des_output, chainName }) => {
      ...
      try { if (window.SCAN_LOG_ENABLED) console.log(`[SWING] Returning top ${maxProviders} routes from ${subResults.length} available routes`); } catch (_) { }
  ...
  dexStrategies.rango = {
    buildRequest: ({ chainName, sc_input_in, sc_output_in, amount_in_big, des_input, symbol_in, symbol_out, SavedSettingData }) => {
      ...
      try {
        ...
        try { if (window.SCAN_LOG_ENABLED) console.warn('[Rango] Failed to apply proxy:', e.message); } catch (_) { }
      }
    ...
    parseResponse: (response, { des_output, chainName }) => {
      ...
      try { if (window.SCAN_LOG_ENABLED) console.log(`[RANGO] Found ${results.length} routes`); } catch (_) { }
      ...
      for (const route of results) {
        try {
          ...
          if (!Number.isFinite(amount_out) || amount_out <= 0) {
            try { if (window.SCAN_LOG_ENABLED) console.warn('[RANGO] Invalid output amount:', route.outputAmount); } catch (_) { }
            continue;
          }
          ...
          try { if (window.SCAN_LOG_ENABLED) console.warn('[RANGO] Error parsing route:', e); } catch (_) { }
          continue;
        }
      }
      ...
      try { if (window.SCAN_LOG_ENABLED) console.log(`[RANGO] Returning top ${maxProviders} routes from ${subResults.length} available routes`); } catch (_) { }
  ...
  dexStrategies.kamino = {
    buildRequest: ({ sc_input_in, sc_output_in, amount_in_big }) => {
      ...
      const params = new URLSearchParams({
        ...
        includeRfq: 'true'
        // ❌ REMOVED: Hardcoded timeoutMs - now managed globally by CONFIG_DEXS.GLOBAL.timeout.kamino
      });
  ...
  dexStrategies.rubic = {
    buildRequest: ({ sc_input_in, sc_output_in, amount_in_big, des_input, chainName }) => {
      ...
      try {
        ...
        try { if (window.SCAN_LOG_ENABLED) console.warn('[Rubic] Failed to apply proxy:', e.message); } catch (_) { }
      }
  ...
  function createFilteredRangoStrategy(dexKey, dexTitle) {
    return {
      ...
      parseResponse: (response, params) => {
        ...
        try { if (window.SCAN_LOG_ENABLED) console.log(`[RANGO-${dexTitle}] Using ${dexTitle} via RANGO: ${amount_out.toFixed(6)} output`); } catch (_) { }
  ...
  function createFilteredRubicStrategy(dexKey, dexTitle) {
    return {
      ...
      parseResponse: (response, params) => {
        ...
        try { if (window.SCAN_LOG_ENABLED) console.log(`[RUBIC-${dexTitle}] Using ${dexTitle} via RUBIC: ${amount_out.toFixed(6)} output`); } catch (_) { }
  ...
  dexStrategies.paraswap5 = dexStrategies.velora5; // Backward compat: paraswap5 -> velora5
   ...
   * @param {string|null} secondary - Secondary strategy name (null if not available)
   * @returns {Object} { selectedStrategy: string, isSecondary: boolean }
   */
  function selectRotationStrategy(dexType, primary, secondary) {
    // If no secondary, always use primary
    if (!secondary) {
      return { selectedStrategy: primary, isSecondary: false };
    }

    // Get or initialize rotation state for this DEX
    if (!DEX_ROTATION_STATE.has(dexType)) {
      DEX_ROTATION_STATE.set(dexType, { counter: 0 });
    }
    ...
    // Round-robin: odd counter = primary, even counter = secondary
  ...
  /**
   * Resolve timeout for a given strategy from global config
   * Priority: Exact match > Wildcard match > Default
   * @param {string} strategyName - Strategy name (e.g., 'velora6', 'lifi-kyber')
   * @returns {number} Timeout in milliseconds
   */
  function resolveStrategyTimeout(strategyName) {
    const globalTimeouts = (root.CONFIG_DEXS?.GLOBAL?.timeout) || {};
    const defaultTimeout = globalTimeouts.default || 15000;

    // 1. Try exact match
    if (globalTimeouts[strategyName]) {
      return globalTimeouts[strategyName];
    }

    // 2. Try wildcard match (e.g., 'lifi-*' matches 'lifi-kyber')
    for (const [pattern, timeout] of Object.entries(globalTimeouts)) {
      if (pattern.endsWith('-*')) {
        const prefix = pattern.slice(0, -2); // Remove '-*'
        if (strategyName.startsWith(prefix + '-')) {
          return timeout;
        }
      }
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        if (dexConfig && dexConfig.disabled === true) {
          try { if (window.SCAN_LOG_ENABLED) console.warn(`[DEX DISABLED] ${String(dexType).toUpperCase()} is disabled in config - skipping request`); } catch (_) { }
          reject({
            statusCode: 0,
      ...
      try {
        ...
        if (isMetaDex && !metaDexEnabled) {
          try { if (window.SCAN_LOG_ENABLED) console.warn(`[META-DEX DISABLED] ${String(dexType).toUpperCase()} is a meta-aggregator but META_DEX is disabled`); } catch (_) { }
          reject({
            statusCode: 0,
      ...
      if (DEX_RESPONSE_CACHE.has(cacheKey)) {
        ...
        if (now - cached.timestamp < DEX_CACHE_TTL) {
          ...
          try { if (window.SCAN_LOG_ENABLED) console.log(`[DEX CACHE HIT] ${dexType.toUpperCase()} (age: ${ageSeconds}s) - Request saved!`); } catch (_) { }
      ...
      if (DEX_INFLIGHT_REQUESTS.has(cacheKey)) {
        ...
        if (!DEX_DEDUP_LOG_TRACKER.has(cacheKey)) {
          try { if (window.SCAN_LOG_ENABLED) console.log(`[DEX DEDUP] ${dexType.toUpperCase()} - Duplicate request prevented!`); } catch (_) { }
      ...
      // ✅ NEW APPROACH: Timeout per strategy dari CONFIG_DEXS
      // Setiap REST API provider (kyber, lifi-kyber, swoop-kyber, dst) punya timeout sendiri
      // Tidak lagi ada global speedScan - timeout sudah optimal per provider di config.js
      const dexLower = String(dexType || '').toLowerCase();

      // Helper function: Get timeout for specific strategy
      const getStrategyTimeout = (strategyKey) => {
        try {
          // Normalize DEX key (handle aliases)
          const aliases = { '0x': 'matcha', 'kyberswap': 'kyber', 'paraswap': 'velora' };
          const normalizedDex = aliases[dexLower] || dexLower;

          // Get DEX config
          const dexConfig = (root.CONFIG_DEXS && root.CONFIG_DEXS[normalizedDex]) || null;
          if (!dexConfig || !dexConfig.timeout) {
            return null; // No timeout config found
          }

          // Get timeout for this specific strategy
          const timeoutMap = dexConfig.timeout;
          const timeout = timeoutMap[String(strategyKey).toLowerCase()];
      ...
      // ❌ REMOVED: Unused DEFAULT_TIMEOUT variable (2026-01-12)
      // All timeouts now managed by CONFIG_DEXS.GLOBAL.timeout with wildcard matching
      ...
      const runStrategy = (strategyName) => new Promise(async (res, rej) => {
        try {
          ...
          // ✅ Get timeout from global config using resolveStrategyTimeout()
          const timeoutMilliseconds = resolveStrategyTimeout(sKey);
          try { if (window.SCAN_LOG_ENABLED) console.log(`⏱️ [${sKey.toUpperCase()} TIMEOUT] ${timeoutMilliseconds}ms`); } catch (_) { }
          ...
          try {
            if (window.SCAN_LOG_ENABLED) console.log(`[${dexType.toUpperCase()} PROXY]`, {
              dexType,
      ...
      try { if (window.SCAN_LOG_ENABLED) console.log(`[DEX TIMEOUT] ${selectedStrategy}: ${strategyTimeout}ms`); } catch (_) { }
  ...
  // ❌ REMOVED: getPriceAltDEX() function (215 lines)
  // Legacy global fallback function that conflicts with CONFIG_DEXS.GLOBAL.allowFallback = false
  // All fallback/rotation logic now handled internally by getPriceDEX() via primary/secondary strategy
  // Date removed: 2026-01-12

  if (typeof App.register === 'function') {
    App.register('Services', { DEX: { dexStrategies, getPriceDEX } });
  }
