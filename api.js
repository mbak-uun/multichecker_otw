...
function calculateAutoVolume(orderbook, maxModal, maxLevels, side) {
  try {
    ...
    try { if (window.SCAN_LOG_ENABLED) console.log('│ 🔍 [AUTO VOLUME] CALCULATION START             │'); } catch(_) {}
    try { if (window.SCAN_LOG_ENABLED) console.log('└─────────────────────────────────────────────────┘'); } catch(_) {}
    try { if (window.SCAN_LOG_ENABLED) console.log('📊 Modal Max:', maxModal); } catch(_) {}
    try { if (window.SCAN_LOG_ENABLED) console.log('📊 Max Levels:', maxLevels); } catch(_) {}
    try { if (window.SCAN_LOG_ENABLED) console.log('📊 Side:', side, side === 'asks' ? '(CEX→DEX/BUY)' : '(DEX→CEX/SELL)'); } catch(_) {}

    // Validation
    if (!orderbook || typeof orderbook !== 'object') {
      try { if (window.SCAN_LOG_ENABLED) console.warn('⚠️  Invalid orderbook data'); } catch(_) {}
      return {
    ...
    if (!Array.isArray(levels) || levels.length === 0) {
      try { if (window.SCAN_LOG_ENABLED) console.warn('⚠️  No orderbook levels available'); } catch(_) {}
      return {
        actualModal: maxModal,
        avgPrice: 0,
        totalCoins: 0,
    ...
    try { if (window.SCAN_LOG_ENABLED) console.log('📚 Orderbook Levels (first', limitedMaxLevels, '):', limitedLevels.map((l, i) => ({
      level: i + 1,
      price: l[0],
      amount: l[1],
    ...
    try { if (window.SCAN_LOG_ENABLED) console.log('🔄 Processing levels...'); } catch(_) {}
    for (let i = 0; i < limitedLevels.length; i++) {
      const [price, amount] = limitedLevels[i];

      // Skip invalid levels
      if (!isFinite(price) || !isFinite(amount) || price <= 0 || amount <= 0) {
        try { if (window.SCAN_LOG_ENABLED) console.warn(`  ⏭️  Level ${i + 1} skipped (invalid):`, { price, amount }); } catch(_) {}
        continue;
      }
      ...
      if (totalUSDT + volumeUSDT > maxModal) {
        ...
        try { if (window.SCAN_LOG_ENABLED) console.log(`  📍 Level ${i + 1} (PARTIAL):`, {
          price,
          amount,
      ...
      try { if (window.SCAN_LOG_ENABLED) console.log(`  ✅ Level ${i + 1} (FULL):`, {
        price,
        amount,
        volumeUSDT: volumeUSDT.toFixed(2),
        totalUSDT: totalUSDT.toFixed(2),
    ...
    // 🔍 DEBUG: Final results
    try { if (window.SCAN_LOG_ENABLED) console.log('┌─────────────────────────────────────────────────┐'); } catch(_) {}
    try { if (window.SCAN_LOG_ENABLED) console.log('│ ✨ [AUTO VOLUME] RESULTS                       │'); } catch(_) {}
    try { if (window.SCAN_LOG_ENABLED) console.log('└─────────────────────────────────────────────────┘'); } catch(_) {}
    try { if (window.SCAN_LOG_ENABLED) console.log('📊 Levels Used:', levelsUsed, '/', limitedMaxLevels); } catch(_) {}
    try { if (window.SCAN_LOG_ENABLED) console.log('💰 Actual Modal:', totalUSDT.toFixed(2), '/', maxModal.toFixed(2)); } catch(_) {}
    try { if (window.SCAN_LOG_ENABLED) console.log('🪙 Total Coins:', totalCoins.toFixed(6)); } catch(_) {}
    try { if (window.SCAN_LOG_ENABLED) console.log('💵 Weighted Avg Price:', avgPrice.toFixed(8), '(for PNL calculation)'); } catch(_) {}
    try { if (window.SCAN_LOG_ENABLED) console.log('💵 Last Level Price:', lastLevelPrice.toFixed(8), '(for display)'); } catch(_) {}
    try { if (window.SCAN_LOG_ENABLED) console.log(''); } catch(_) {}
...
// ❌ REMOVED: getPriceAltDEX() wrapper (2026-01-12)
// Legacy global fallback removed - use getPriceDEX() with primary/secondary rotation instead
