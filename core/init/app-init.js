...
$(document).ready(function () {
    ...
    setTimeout(function () {
        try {
            ...
            try { if (window.SCAN_LOG_ENABLED) console.warn('[APP-INIT] Migration failed:', e.message); } catch(_) {}
        }
    }, 50);
    ...
    setTimeout(function () {
        try {
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log('[APP-INIT] 🔍 Restoring checkboxes...'); } catch(_) {}
            try { if (window.SCAN_LOG_ENABLED) console.log('[APP-INIT]   ∟ Current mode:', currentMode); } catch(_) {}
            try { if (window.SCAN_LOG_ENABLED) console.log('[APP-INIT]   ∟ Filter key:', currentFilterKey); } catch(_) {}
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log('[APP-INIT]   ∟ Loaded preferences:', prefs); } catch(_) {}

            // ✅ VALIDATION: Enforce mutually exclusive AUTO VOL and AUTO LEVEL
            if (prefs.autoVol && prefs.autoLevel) {
                try { if (window.SCAN_LOG_ENABLED) console.warn('[APP-INIT] ⚠️  Invalid state: both autoVol and autoLevel are true'); } catch(_) {}
                try { if (window.SCAN_LOG_ENABLED) console.warn('[APP-INIT] Prioritizing AUTO VOL, disabling AUTO LEVEL'); } catch(_) {}
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log(`[APP-INIT] ✅ Checkbox controls restored from ${filterKey}:`, prefs); } catch(_) {}
        } catch (e) {
            try { if (window.SCAN_LOG_ENABLED) console.warn('[APP-INIT] Failed to restore checkbox settings:', e.message); } catch(_) {}
        }
    ...
    try {
        ...
        } else if (typeof setScanUIGating === 'function') {
            // Ensure UI is active when idle
            setScanUIGating(false);
        }
    } catch (_) { }
    ...
    function renderChainLinks(activeKey = 'all') {
        ...
        Object.keys(CONFIG_CHAINS || {}).forEach(chainKey => {
            ...
            const style = isActive ? 'width:40px' : '';
            const width = isActive ? 40 : 32;
