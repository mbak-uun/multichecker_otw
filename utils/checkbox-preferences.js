...
(function () {
    ...
    function getCheckboxPreferences() {
        try {
            ...
            try { if (window.SCAN_LOG_ENABLED) console.warn('[CHECKBOX-PREFS] Failed to get preferences:', e.message); } catch(_) {}
            return {
                autoScroll: false,
                autoRun: false,
                autoVol: false,
    ...
    function saveCheckboxPreference(key, value) {
        try {
            ...
            if (typeof saveToLocalStorage === 'function') {
                ...
                try { if (window.SCAN_LOG_ENABLED) console.log(`[CHECKBOX-PREFS] Saved ${key}=${value} to ${filterKey}`); } catch(_) {}
            }
        } catch (e) {
            try { if (window.SCAN_LOG_ENABLED) console.warn('[CHECKBOX-PREFS] Failed to save preference:', e.message); } catch(_) {}
        }
    }
    ...
    function migrateCheckboxPreferences() {
        try {
            ...
            if (!needsMigration) {
                try { if (window.SCAN_LOG_ENABLED) console.log('[CHECKBOX-PREFS] No migration needed'); } catch(_) {}
                return;
            }

            try { if (window.SCAN_LOG_ENABLED) console.log('[CHECKBOX-PREFS] Migrating old preferences to per-chain storage...'); } catch(_) {}
            ...
            try { if (window.SCAN_LOG_ENABLED) console.log('[CHECKBOX-PREFS] ✅ Migration complete'); } catch(_) {}
        } catch (e) {
            try { if (window.SCAN_LOG_ENABLED) console.warn('[CHECKBOX-PREFS] Migration failed:', e.message); } catch(_) {}
        }
    }
